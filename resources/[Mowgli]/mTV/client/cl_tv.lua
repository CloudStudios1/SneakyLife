	...
	TriggerEvent("cTV:ToggleNui", {
        ...
        for k, v in pairs(Config['Objects']) do
            ...
            if not DoesEntityExist(obj) then 
                return ESX.ShowNotification("~r~Vous devez être à côté d'un écran.") 
            end
            if youtubeID == nil then 
                return ESX.ShowNotification("~r~Vous devez entrer un lien valide.") 
            end

            TriggerServerEvent('clp_tv:add', youtubeID, v['Object'], GetEntityCoords(obj), v['Scale'], v['Offset'], volume)
            ESX.ShowNotification('/volume (0-10): ~b~Changer le volume~s~.\n/sync: ~b~Sync la vidéo~s~.\n/destroy: ~b~Stopper la vidéo~s~.')
            break
        end
	end)
end)
