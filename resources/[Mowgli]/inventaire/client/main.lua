        ...
        elseif data.item.type == "item_chaussures" then
            TriggerEvent('Sneakyskinchanger:getSkin', function(skin)
                ...
                if shoes then
                    ...
                    ESX.ShowNotification("Vous avez ~b~équipé~s~ vôtre ~b~"..data.item.label)
                    TriggerEvent('Sneakyskinchanger:loadClothes', skin, { 
                        ["shoes_1"] = data.item.skins["shoes_1"],
                        ["shoes_2"] = data.item.skins["shoes_2"]})
                end
                        ...
                        ESX.TriggerServerCallback('Sneakyesx_skin:getPlayerSkin', function(skina)
                            TriggerEvent('Sneakyskinchanger:getSkin', function(skinb)
                                if skina.tshirt_1 ~= skinb.tshirt_1 then
                                    TriggerEvent('Sneakyskinchanger:loadClothes', skinb, {['tshirt_1'] = skina.tshirt_1, ['tshirt_2'] = skina.tshirt_2})
                                else
                                    TriggerEvent('Sneakyskinchanger:loadClothes', skinb, {['tshirt_1'] = -1, ['tshirt_2'] = 0})
                                end
                            end)
                        end)
                    end
                        ...
                        ESX.TriggerServerCallback('Sneakyesx_skin:getPlayerSkin', function(skina)
                            TriggerEvent('Sneakyskinchanger:getSkin', function(skinb)
                                if skina.glasses_1 ~= skinb.glasses_1 then
                                    TriggerEvent('Sneakyskinchanger:loadClothes', skinb, {['glasses_1'] = skina.glasses_1, ['glasses_2'] = skina.glasses_2})
                                else
                                    TriggerEvent('Sneakyskinchanger:loadClothes', skinb, {['glasses_1'] = -1, ['glasses_2'] = 0})
                                end
                            end)
                        end)
                    end
                        ...
                        ESX.TriggerServerCallback('Sneakyesx_skin:getPlayerSkin', function(skina)
                            TriggerEvent('Sneakyskinchanger:getSkin', function(skinb)
                                if skina.helmet_1 ~= skinb.helmet_1 then
                                    TriggerEvent('Sneakyskinchanger:loadClothes', skinb, {['helmet_1'] = skina.helmet_1, ['helmet_2'] = skina.helmet_2})
                                else
                                    TriggerEvent('Sneakyskinchanger:loadClothes', skinb, {['helmet_1'] = -1, ['helmet_2'] = 0})
                                end
                            end)
                        end)
...
RegisterNUICallback("GiveItem", function(data, cb)
    ...
    if foundPlayer then
        ...
        if data.item.type == "item_tenue" or data.item.type == "item_chaussures" or data.item.type == "item_calque" or data.item.type == "item_montre" or data.item.type == "item_chaine" or data.item.type == "item_masque" or data.item.type == "item_pantalon" or data.item.type == "item_chapeau" or data.item.type == "item_sac" or data.item.type == "item_tshirt" then
...
function loadPlayerInventory(result)
    ...
    elseif result == 'clothe' then 
        ...
        ESX.TriggerServerCallback('SneakyLife:GetOutfit', function(Vetement)
            tenue, chaussures, masque, pantalon, tshirt, lunettes, chapeau, sac, chaine, calque, montre, torse, bracelet, oreille = {}, {}, {}, {}, {}, {}, {}, {}, {}, {},{}, {}, {}, {}
...
function GetNearbyPlayer(solo, other)
    ...
    ShowAboveRadarMessage("Appuyer sur ~g~E~s~ pour valider.Appuyer sur ~b~A~s~ pour changer de cible.Appuyer sur ~r~X~s~ pour annuler.")
    Citizen.Wait(100)
    local cBase = 1
    cWait = true
