    ...
    if Garage.Menu then
        ...
        Citizen.CreateThread(function()
			while Garage.Menu do
                ...
                RageUI.IsVisible(RMenu:Get('garage', 'submycars'), true, true, true, function()
                ...
                RageUI.IsVisible(RMenu:Get('garage', 'suboption'), true, true, true, function()
                    local c = vehSelected
                    ...
                    elseif c.parked == "0" or c.parked == 0 then
                        ...
                        if not DoesBlipExist(PersoCarblip) then
                            RageUI.Button("Rappeler le véhicule", false, {RightLabel = "~g~$"..prc}, true, function(Hovered, Active, Selected)
                                if Selected then
                                    AtlantisRPEvent("pGarage:UpdateParkedStatus", c.plate, prc)
                                    RageUI.GoBack()
                    ...
                    RageUI.Button("Renommer", false, {}, true, function(Hovered, Active, Selected)
                        if Selected then
                            ...
                            if name ~= nil then
                                AtlantisRPEvent("pGarare:RenameVeh", c.plate, name)
                                ...
                                if Selected then
                                    for k,v in pairs(ESX.Game.GetVehicles()) do
                                        ...
                                        if plate == c.plate then
                                            return ESX.ShowNotification("~r~Erreur~s~Vous ne pouvez pas dupliquer sur ce serveur.")
                                        end
                                    end
                                    AtlantisRPEvent("pGarage:RequestSpawn", c.plate, vector3(v.pos.x, v.pos.y, v.pos.z), v.heading)
                                end
                                if Active then
                            ...
                            RageUI.Button("Place #"..k.." - ~HUD_COLOUR_BLUELIGHT~("..plate..")", false, {RightLabel = "~r~Ranger →"}, true, function(Hovered, Active, Selected)
                                if Selected then
                                    if veh == nil and not veh and not DoesEntityExist(veh) then
                                        ESX.ShowNotification("~r~Erreur~s~Veuillez ressayer plus tard !")
                                    else
                                        ...
                                        ESX.TriggerServerCallback('pGarage:StockVehicle',function(valid)
                                            if(valid) then
                                                ...
                                                AtlantisRPEvent("pGarage:UpdateParkedStatus", vehProps.plate)
