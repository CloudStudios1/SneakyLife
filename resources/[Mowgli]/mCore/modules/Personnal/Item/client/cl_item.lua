...
AddEventHandler('Sneakyesx_armour:armor', function()
	...
	if GetPedArmour(plyPed) == 100 then
		...
		AnimGilet()
		AtlantisRPEvent('Sneakyesx_armour:armorremove')
...
AddEventHandler('Sneakyesx_repairkit:onUse', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			if IgnoreAbort then
				AtlantisRPEvent('Sneakyesx_repairkit:removeKit')
			end
...
RegisterNetEvent("SneakyLife:Uselockpick")
AddEventHandler("SneakyLife:Uselockpick",function()
    ...
    if ProgressBarExists() then return ESX.ShowNotification("~r~Vous ne pouvez pas faire cela.") end
	if DoesEntityExist(vehicle) then
		...
		Citizen.CreateThread(function()
            ...
            if chance >= 70 then
                TriggerServerEvent("AtlantisRPremovelockpick")
            end
		end)
	else
...
RegisterNetEvent("mCore:useDiable")
AddEventHandler("mCore:useDiable",function()
    ...
    while IsEntityAttachedToEntity(diable, PlayerPedId()) do
        ...
        if IsControlJustPressed(0, 73) then
            ...
            if PlayerWeight < 45 then
                TriggerServerEvent("mCore:diableSystem","diable")
                ClearPedTasksImmediately(PlayerPedId())
                DetachEntity(diable, true, true)
                DeleteEntity(diable)
            else
...
function AttachCaseToPed(item_name)
    if not DontSpam then
        ...
        if exports["mCore"]:ProgressBarExists() then
...
function RepairVehicle(type, Entity)
    ...
    exports["mCore"]:ProgressBar("Réparation en cours", 58, 173, 0, 200, timetowait)
    if type == 2 then
...
AddEventHandler("SneakyLife:UseItemTires", function()
    ...
    SetVehicleUndriveable(Entity, true)
    exports["mCore"]:ProgressBar("Changement de la roue", 58, 173, 0, 200, TimeScenerio)
...
AddEventHandler("SneakyLife:UseItemWash", function()
    ...
    SetVehicleUndriveable(Entity, true)
    exports["mCore"]:ProgressBar("Nettoyage en cours", 58, 173, 0, 200, TimeWash)
