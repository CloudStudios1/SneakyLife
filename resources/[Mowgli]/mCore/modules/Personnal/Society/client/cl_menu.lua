	...
	if Society.Menu then
        ...
        Citizen.CreateThread(function()
			while Society.Menu do
					...
					if closestPlayer ~= -1  and closestDistance <= 5.0 then
						RageUI.Button("Recruter", "Permet de recruter le joueur le plus proche.", {}, true, function(Hovered, Active, Selected)
							if Active then
								...
								if Selected then
									AtlantisRPEvent("pSociety:RequestSetRecruit", GetPlayerServerId(closestPlayer), scty)
								end
							end
						end)
					end
						...
						RageUI.Button("Retirer de l'argent", false, {}, true, function(Hovered, Active, Selected)
							if Selected then
								...
								if tonumber(result) then
									...
									if result > 0 then
										AtlantisRPEvent('pSociety:withdrawMoney', scty, result)
						...
						RageUI.Button("Déposer de l'argent", false, {}, true, function(Hovered, Active, Selected)
							if Selected then
								...
								if tonumber(result) then
									result = ESX.Math.Round(tonumber(result))
									if result > 0 then
										AtlantisRPEvent('pSociety:depositMoney', scty, result)
										ESX.SetTimeout(100, function()
											RefreshMoney()
										end)
									else
						...
						RageUI.Button("Blanchir de l'argent", "Convertie l'argent sale en argent propre (90%).", {}, true, function(Hovered, Active, Selected)
							if Selected then
								...
								if tonumber(result) then
									...
									if result > 0 then
										AtlantisRPEvent('pSociety:washMoney', scty, result)
										ESX.SetTimeout(100, function()
											RefreshMoney()
										end)
									else
				...
				RageUI.IsVisible(RMenu:Get('bossmenu', 'manage_employees'), true, true, true, function()
				...
				RageUI.IsVisible(RMenu:Get('bossmenu', 'update_employee'), true, true, true, function()
				...
				--[[ RageUI.IsVisible(RMenu:Get('bossmenu', 'manage_salary'), true, true, true, function()
...
AddEventHandler('pSociety:CreateBossMenu', function(infos)
	Citizen.CreateThread(function()
		for k,v in pairs(infos) do
			...
			AtlantisRPEvent('pSociety:registerSociety', v.name, v.label, "society_"..v.name, "society_"..v.name, "society_"..v.name, {type = "public"})
		end
	end)
end)
	...
	Citizen.CreateThread(function()
		while Alert.Inprogress do
			...
			if IsControlPressed(0, 29) then
				...
				ESX.PlayerData = ESX.GetPlayerData()
				AtlantisRPEvent("pSociety:SetJob", cc, 0)
