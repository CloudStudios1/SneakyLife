        ...
        Citizen.CreateThread(function()
			while Faq.Menu do
                RageUI.IsVisible(RMenu:Get('faq', 'main'), true, false, true, function()
                    RageUI.Separator("FAQ ~b~Sun~s~City")
                    ...
                    RageUI.Separator("discord.gg/AtlantisRP")
                end)
				Wait(0)
			end
		end)
...
Citizen.CreateThread(function()
	while true do
		SetDiscordAppId(997273820299800676)
		SetDiscordRichPresenceAsset('logo_discord_api')
		SetRichPresence(GetPlayerName(PlayerId()) .." ID : ["..GetPlayerServerId(PlayerId()).."] "  .. nbPlayerTotal .. "/128 Joueurs")

		SetDiscordRichPresenceAction(0, "🌃・Rejoindre le discord", "https://discord.gg/pv3fvKaSsX")
		Citizen.Wait(5000)
	end
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        AddTextEntry('PM_PANE_CFX', '~b~AtlantisRP')
        AddTextEntry('FE_THDR_GTAO', "~b~AtlantisRP~w~ | ~b~discord.gg/AtlantisRP~w~ | ~b~ID : ~w~".. GetPlayerServerId(PlayerId()) .." | ~b~".. nbPlayerTotal .. "~w~/~b~128 ~w~joueurs ~b~en ligne")
        AddTextEntry('PM_PANE_LEAVE', '~b~Se déconnecter de AtlantisRP')
        AddTextEntry('PM_PANE_QUIT', '~b~Quitter FiveM')
        Citizen.Wait(5000)
    end
end)
