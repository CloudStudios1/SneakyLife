...
function OpenCreatorMenu()
    if b.Menu then
        ...
        RMenu.Add('creator', 'identity',
            RageUI.CreateSubMenu(RMenu:Get("creator", "main"), "", "Création d'identité de ~b~Sun~s~City."))
        RMenu.Add('creator', 'character',
            RageUI.CreateSubMenu(RMenu:Get("creator", "main"), "", "Création d'identité de ~b~Sun~s~City."))
        RMenu.Add('creator', 'characteroptionsmain', RageUI.CreateSubMenu(RMenu:Get("creator", "character"), "",
            "Création d'identité de ~b~Sun~s~City."))
        ...
        RMenu.Add('creator', 'characteroptions_a',
            RageUI.CreateSubMenu(RMenu:Get("creator", "characteroptionsaccessories"), "",
                "Création d'identité de ~b~Sun~s~City."))
        RMenu:Get('creator', 'main'):SetSubtitle("Création d'identité de ~b~Sun~s~City.")
            ...
            while b.Menu do
                RageUI.IsVisible(RMenu:Get('creator', 'main'), true, true, true, function()
                    ...
                    if sIdentityEnd then
                        RageUI.Button("Créer son identité.", nil, {
                            RightLabel = "→"
                        }, true, function(k, l, m)
                        end, RMenu:Get("creator", "identity"))
                    else
                        RageUI.Button("Vous avez terminé votre identité.",
                            "Prénom : " .. sDataIdentity.surname .. "Nom : " .. sDataIdentity.name .. "Taille : " ..
                                sDataIdentity.height .. "Date de naissance : " .. sDataIdentity.birthday, {
                                RightLabel = "→"
                            }, false, function()
                            end)
                    end
                    if sCharEnd then
                        RageUI.Button("Créer son personnage.", nil, {
                            RightLabel = "→"
                        }, true, function(k, l, m)
                        end, RMenu:Get("creator", "character"))
                    else
                        RageUI.Button("Vous avez terminé votre personnage.", nil, {
                            RightLabel = "→"
                        }, false, function()
                        end)
                    end
                    if not sIdentityEnd and not sCharEnd then
                        RageUI.Button("Commencer l'aventure.", nil, {
                            RightLabel = "→",
                            Color = {
                ...
                RageUI.IsVisible(RMenu:Get('creator', 'identity'), true, true, true, function()
                    FreezeEntityPosition(GetPlayerPed(-1), true)
                    RageUI.Button("Prénom", nil, {
                        RightLabel = sDataIdentity.surname
                    }, true, function(k, l, m)
                        if m then
                            local o = sKeyboardInput("Prénom", "", 20)
                            if o ~= nil then
                                sDataIdentity.surname = o
                            end
                        end
                    end)
                    RageUI.Button("Nom", nil, {
                        RightLabel = sDataIdentity.name
                    }, true, function(k, l, m)
                        if m then
                            local p = sKeyboardInput("Nom", "", 20)
                            if p ~= nil then
                                sDataIdentity.name = p
                            end
                        end
                    end)
                    RageUI.Button("Taille", nil, {
                    ...
                    RageUI.Button("Date de naissance", nil, {
                    ...
                    RageUI.List("Sexe", {{
                        ...
                        if u then
                            local w = IdSelected;
                            if v == 1 then
                                sDataIdentity.sexe = "Homme"
                            else
                                sDataIdentity.sexe = "Femme"
                            end
                        end
                        sIndexSex = v
                    end)
                    if sDataIdentity.name ~= "" and sDataIdentity.surname ~= "" and sDataIdentity.height ~= "" and sDataIdentity.birthday ~= "" then
                        RageUI.Button("Confirmer son identité.", nil, {
                            RightLabel = "→",
                            Color = {
                                HightLightColor = {0, 158, 255, 160},
                                BackgroundColor = {0, 178, 255, 160}
                            ...
                            if m then
                                sIdentityEnd = false;
                                RageUI.GoBack()
                                TriggerServerEvent('core:CreateIdentity', sDataIdentity.name, sDataIdentity.surname, sDataIdentity.height, sDataIdentity.birthday)
                                ESX.ShowNotification("~b~AtlantisRP~s~LifeIdentité créer avec succès.")
                            end
                        end)
                    end
                end)
                RageUI.IsVisible(RMenu:Get('creator', 'character'), true, true, true, function()
                    ...
                    RageUI.Button("Terminer son personnage.", nil, {
                        RightLabel = "→",
                        Color = {
                            HightLightColor = {0, 158, 255, 160},
                            BackgroundColor = {0, 178, 255, 160}
                ...
                RageUI.IsVisible(RMenu:Get('creator', 'characteroptions_a'), true, true, true, function()
                    ...
                    for z, y in pairs(Components) do
                        if y.name == sData then
                            for B = -1, ComponentsMax[sData] do
                                RageUI.Button(y.label .. " N°" .. B, nil, {
                                    RightLabel = "→"
                                }, true, function(k, l, m)
                                    if l then
