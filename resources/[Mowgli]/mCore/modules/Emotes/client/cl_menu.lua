        ...
        if (U == "MaleScenario" or "Scenario") and
            ...
            if U == "MaleScenario" then
                if GetEntityModel(PlayerPedId()) ==
                    ...
                    ESX.ShowNotification("~r~Cet emote est réservé aux hommes.")
                end
                return
...
local function AnimShared(Infos, ListId)
    ...
    if target then
        TriggerServerEvent("mCore:RequestEmote", GetPlayerServerId(target), ListId, Infos[3])
    else
...
AddEventHandler("ClientEmoteRequestReceive", function(a0, a1)
    ...
    ESX.ShowNotification("~y~X~w~ accepter, ~r~L~w~ refuser (~g~"..a1.."~w~)")
    local isRequestAnim = 1;
    while isRequestAnim <= 100 do
        Citizen.Wait(5)
        isRequestAnim = isRequestAnim + 1
        if IsControlJustPressed(1, 120) then
            TriggerServerEvent("mCore:ValidServer", GetPlayerServerId(GetClosestPlayer(3)), a0)
        elseif IsControlJustPressed(1, 182) then
            ESX.ShowNotification("~r~Emote refusée.")
        end
    end
end)
    ...
    if Animations.Menu then
        ...
        Citizen.CreateThread(function()
			while Animations.Menu do
                RageUI.IsVisible(RMenu:Get('animation', 'main'), true, true, true, function()
                ...
                RageUI.IsVisible(RMenu:Get('animation', 'ai'), true, true, true, function()
                ...
                RageUI.IsVisible(RMenu:Get('animation', 'aj'), true, true, true, function()
                    if mainAJ then
                        ...
                        for a8, ar in ipairs(l) do
                            if Infos.NameMenu == "Bind l'animation" and not ar.Assign then
                                RageUI.Button("Touches n°" .. a8,"Assigner l'animation à cette touche",{RightLabel = "→"}, true, function(h,a,s)
                                    if s then
                                        RageUI.GoBack()
                                        ESX.ShowNotification("Vous avez bind l'animations: ~b~" ..Infos.ListId .. "~s~ sur la touche: ~b~" ..a8 .. "~s~.")
                            ...
                            elseif Infos.NameMenu == "Supprimer l'animation des binds" and ar.Assign then
                                RageUI.Button("Touches n°" .. a8,"Supprimer l'animation à cette touche",{RightLabel = "→"}, true, function(h,a,s)
                                    if s then
                                        ESX.ShowNotification("~r~Vous avez supprimé une animations sur la touche: ~b~" ..a8 .. "~s~.")
                                        a7(a8, false)
                                    end
                                end)
                            end
...
RegisterKeyMapping("animreturn", "Ouvrir le menu des animations (Bind)","keyboard", "K")
RegisterCommand("animreturn", function() 
    ClearPedTasks(GetPlayerPed(-1))
    o = false;
    p()
end)
