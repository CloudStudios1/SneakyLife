...
function OpenTestRageUIMenu()
    ...
    if Test.Menu then 
        ...
        Citizen.CreateThread(function()
			while Test.Menu do
                RageUI.IsVisible(RMenu:Get('staff', 'main'), true, true, true, function()
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'reportsmain'), true, true, true, function()
                    ...
                    if not NamesRp then
                        for sender, infos in pairs(localReportsTable) do
                            if infos.taken then
                                if hideTakenReports == false then
                                    RageUI.Button(getIsTakenDisplay(infos.taken).." "..infos.id .." - "..infos.name,"~b~Pris en charge par :~s~ "..infos.takenBy.."~b~Motif :~s~ "..infos.reason.."~b~Date : ~s~"..infos.timeElapsed[2].."h"..infos.timeElapsed[1].."m", { RightLabel = "→" }, true, function(_, _, s)
                                        if s then
                                ...
                                RageUI.Button(getIsTakenDisplay(infos.taken).." "..infos.id.." - "..infos.name,"~b~Pris en charge par :~s~ Personne~b~Motif :~s~ "..infos.reason.."~b~Date : ~s~"..infos.timeElapsed[2].."h"..infos.timeElapsed[1].."m", { RightLabel = "→" }, true, function(_, _, s)
                                    if s then
                                        selectedReport = sender
                                    end
                                end,RMenu:Get("staff","reportsaction"))
                        ...
                        for sender, infos in pairs(localReportsTable) do
                            ...
                            if infos.taken then
                                if hideTakenReports == false then
                                    RageUI.Button(getIsTakenDisplay(infos.taken).." "..infos.id .." - "..identityrp,"~b~Pris en charge par :~s~ "..infos.takenBy.."~b~Motif :~s~ "..infos.reason.."~b~Date : ~s~"..infos.timeElapsed[2].."h"..infos.timeElapsed[1].."m", { RightLabel = "→" }, true, function(_, _, s)
                                        if s then
                                            selectedReport = sender
                                        end
                                    end,RMenu:Get("staff","reportsaction"))
                                ...
                                RageUI.Button(getIsTakenDisplay(infos.taken).." "..infos.id.." - "..identityrp,"~b~Pris en charge par :~s~ Personne~b~Motif :~s~ "..infos.reason.."~b~Date : ~s~"..infos.timeElapsed[2].."h"..infos.timeElapsed[1].."m", { RightLabel = "→" }, true, function(_, _, s)
                                    if s then
                                        selectedReport = sender
                                    end
                                end,RMenu:Get("staff","reportsaction"))
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'reportsaction'), true, true, true, function()
                    if localReportsTable[selectedReport] ~= nil then
                        ...
                        if not localReportsTable[selectedReport].taken then
                            RageUI.Button("Prendre le ticket", nil, {}, true, function(_, _, s)
                                if s then
                                    ...
                                    AtlantisRPEvent("Jojo:TeleportAdmin", 'goto', selectedReport)
                                end
                            end)
                        end
                        RageUI.Button("Envoyer un message", nil, {}, true, function(h, a, s)
                            if s then
                                ...
                                if msg ~= nil and msg ~= '' and msg ~= 'nil' then
                                    AtlantisRPEvent("core:Message", selectedReport, msg)
                                else
                        ...
                        RageUI.Button('Revive le joueur' , nil, {},true, function(h, a, s)
                            if s then
                                AtlantisRPEvent('SneakyLife:AdminRevive', selectedReport)
                            end
                        end)
                        ...
                        RageUI.Button('Bring', nil, {},true, function(h, a, s)
                            if s then
                                AtlantisRPEvent("Jojo:TeleportAdmin", 'bring', selectedReport)
                            end
                        end)
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'playerslist'), true, true, true, function()
                ...
                local labelright = ""
                RageUI.IsVisible(RMenu:Get('staff', 'playersaction'), true, true, true, function()
                    ...
                    RageUI.Button("Envoyer un message privé", nil, {RightLabel = ""}, true, function(Hovered, Active, Selected)
                        if (Selected) then
                            ...
                            msg = tostring(msg)
                            if msg ~= nil and msg ~= '' and msg ~= 'nil' then
                                AtlantisRPEvent("core:Message", idjoueur, msg)
                            else
                                ESX.ShowNotification("~r~Erreur\n~s~Merci d'écrire le message correctement")
                            end
                        end
                    ...
                    RageUI.Button('Se téléporter sur le joueur', nil, {},true, function(h, a, s)
                        if s then
                            AtlantisRPEvent("Jojo:TeleportAdmin", 'goto', IdSelected)
                        end
                    end)
            
                    RageUI.Button("Téléporter le joueur sur vous", nil, {}, true, function(Hovered, Active, Selected)
                        if (Selected) then
                            AtlantisRPEvent("Jojo:TeleportAdmin", 'bring', IdSelected)
                        end
                    end)
                    RageUI.List("Informations", information, informationindex, nil, {}, true, function(Hovered, Active, Selected, Index) 
                        if (Selected) then 
                            ...
                            TriggerServerEvent('AtlantisRPgetinformation', idjoueur, Index)
                        end 
                        informationindex = Index 
                    end)
                    RageUI.Button('Réanimer' , nil, {},true, function(h, a, s)
                        if s then
                            AtlantisRPEvent('SneakyLife:AdminRevive', IdSelected)
                        end
                    end)
                    RageUI.Button("Avertissement", nil, { RightLabel = ""}, true, function(h,a,s)
                        if s then
                            ...
                            if Reason == "" or Reason == nil then
                                ...
                                local idjoueur = IdSelected
                                ESX.ShowNotification("~b~AtlantisRP~s~\nVous venez de warn ~c~"..GetPlayerName(GetPlayerFromServerId(idjoueur)).."~s~ avec comme raison : ~y~"..Reason)
                                AtlantisRPEvent("AtlantisRPwarnPlayer", idjoueur, Reason)
                                AtlantisRPEvent("AtlantisRPLog:Avertissement", idjoueur, Reason)
                            end
                        end
                    end)
                    RageUI.Button("Liste des avertissements", nil, { RightLabel = ""}, true, function(h,a,s)
                        if s then
                            local idjoueur = IdSelected
                            ESX.TriggerServerCallback('AtlantisRPGetHistoWarn', function(warns)
                                wList = warns
                            end,idjoueur)
                        end
                    ...
                    RageUI.Button("Liste des bans", nil, { RightLabel = ""}, true, function(h,a,s)
                        if s then
                            local idjoueur = IdSelected
                            ESX.TriggerServerCallback('AtlantisRPGetHistoBan', function(ban)
                                bList = ban
                            end,idjoueur)
                        end
                    ...
                    RageUI.Button("Kick", nil, {RightLabel = ""}, true, function(Hovered, Active, Selected)
                        if (Selected) then
                            ...
                            if reason ~= nil and reason ~= '' and reason ~= 'nil' then
                                AtlantisRPEvent("SneakyLife:Kick", idjoueur, reason)
                            else
                    ...
                    RageUI.Button('Freeze', nil, {},true, function(h, a, s)
                        if s then
                            AtlantisRPEvent("Jojo:TeleportAdmin", 'freeze', IdSelected)
                        end
                    end)
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'optionsplayer'), true, true, true, function()
                    if (ESX.GetPlayerData()['group']) == "_dev" or (ESX.GetPlayerData()['group']) == "gs" or (ESX.GetPlayerData()['group']) == "superadmin" then
                    ...
                    RageUI.Button("Wipe", nil, {RightLabel = ""}, true, function(Hovered, Active, Selected)
                        if (Selected) then
                            ...
                            if message == "oui" then
                                AtlantisRPEvent("sAdmin:wipePlayer", IdSelected)
                            end
                        end
                    end)
                    if (ESX.GetPlayerData()['group']) == "_dev" or (ESX.GetPlayerData()["group"]) == "gs" then
                        RageUI.List("Changer le rang", group, groupindex, nil, {}, true, function(Hovered, Active, Selected, Index) 
                            if (Selected) then 
                                local idjoueur = IdSelected
                                if Index == 1 then 
                                    AtlantisRPEvent('AtlantisRPsetgroup', idjoueur, "group", "user")
                                elseif Index == 2 then
                                    AtlantisRPEvent('AtlantisRPsetgroup', idjoueur, "group", "admin")
                                elseif Index == 3 then
                                    AtlantisRPEvent('AtlantisRPsetgroup', idjoueur, "group", "superadmin")
                                elseif Index == 4 then
                                    AtlantisRPEvent('AtlantisRPsetgroup', idjoueur, "group", "gs")
                                elseif Index == 5 then
                                    if (ESX.GetPlayerData()['group']) == "_dev" then
                                        AtlantisRPEvent('AtlantisRPsetgroup', idjoueur, "group", "_dev")
                                    end
                                end 
                            end 
                            groupindex = Index 
                        ...
                        RageUI.List("Changer les permissions", perms, permsindex, nil, {}, true, function(Hovered, Active, Selected, Index) 
                            if (Selected) then 
                                local idjoueur = IdSelected
                                if Index == 1 then 
                                    AtlantisRPEvent('AtlantisRPsetperms', idjoueur, "level", "0")
                                elseif Index == 2 then
                                    AtlantisRPEvent('AtlantisRPsetperms', idjoueur, "level", "1")
                                elseif Index == 3 then
                                    AtlantisRPEvent('AtlantisRPsetperms', idjoueur, "level", "2")
                                elseif Index == 4 then
                                    AtlantisRPEvent('AtlantisRPsetperms', idjoueur, "level", "3")
                                elseif Index == 5 then
                                    AtlantisRPEvent('AtlantisRPsetperms', idjoueur, "level", "4")
                                end 
                            end 
                            permsindex = Index 
                        end)
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'remboursement'), true, true, true, function()
                    ...
                    RageUI.Button("Donner de l'argent", nil, {RightLabel = ""}, true, function(Hovered, Active, Selected)
                        if (Selected) then
                            ...
                            if amount ~= nil and amount > 0 then
                                AtlantisRPEvent('AtlantisRPsMenu:GiveMoney', amount, idjoueur)
                                AtlantisRPEvent("AtlantisRPLog:Givecash", IdSelected, amount)
                            else
                    ...
                    RageUI.Button("Donner de l'argent en banque", nil, {RightLabel = ""}, true, function(Hovered, Active, Selected)
                        if (Selected) then
                            ...
                            local idjoueur = IdSelected
                            if amount ~= nil and amount > 0 then
                                AtlantisRPEvent('AtlantisRPsMenu:GiveBankMoney', amount, idjoueur)
                                AtlantisRPEvent("AtlantisRPLog:Givebank", IdSelected, amount)
                            else
                                ESX.ShowNotification("~r~Erreur\n~s~Le montant est incorrect")
                            end
                        end
                    ...
                    RageUI.Button("Donner de l'argent sale", nil, {RightLabel = ""}, true, function(Hovered, Active, Selected)
                        if (Selected) then
                            ...
                            local idjoueur = IdSelected
                            if amount ~= nil and amount > 0 then
                                AtlantisRPEvent('AtlantisRPsMenu:GiveDirtyMoney', amount, idjoueur)
                                AtlantisRPEvent("AtlantisRPLog:Givedirty", IdSelected, amount)
                            else
                                ESX.ShowNotification("~r~Erreur\n~s~Le montant est incorrect")
                            end
                        end
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'itemlist'), true, true, true, function()
                        ...
                        if starts(itemInfos.label:lower(), filterArray[filter]:lower()) then
                            RageUI.Button(itemInfos.label.." - ~c~"..itemInfos.name, nil, { RightLabel = "→" }, true, function(_, _, s)
                                if s then
                                    ...
                                    local amount = CustomAmount2()
                                    if amount ~= nil and amount > 0 then
                                        TriggerServerEvent("SneakyLife:GiveItem", idjoueur, itemInfos.name, amount)
                                    end
                                end
                            end)
                        end
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'warnlist'), true, true, true, function()
                    ...
                    for i = 1, #wList, 1 do
                        ...
                        RageUI.Button("Raison : ~b~"..v.reason,"Par : ~y~".. v.moderator.."~s~ le "..v.date,{RightLabel = "~r~Supprimer~s~ →"},true,function(_, _, Selected)
                            if Selected then
                                if (ESX.GetPlayerData()['group']) == "_dev" then
                                    AtlantisRPEvent("sAdmin:deleteWarnPlayer", v)
                                    RageUI.GoBack()
                                else
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'banlist'), true, true, true, function()
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'joblist'), true, true, true, function()
                            ...
                            for k,i in pairs(v.grade) do
                                RageUI.Button(v.name.." - ~c~["..k.."] "..i.label, nil, {RightLabel = ""}, true,function(h,a,s)
                                    if s then
                                        ...
                                        AtlantisRPEvent("AtlantisRPAdmin:setjob",idjoueur, v.name, k)
                                        AtlantisRPEvent('AtlantisRPLog:Changerlejob', IdSelected, v.name, k)
                                    end
                                end)
                            end
                        end
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'grouplist'), true, true, true, function()
                    RageUI.Button("~y~Filtrer",nil,{RightLabel = "🔍"},true,function(_, _, Selected)
                        if Selected then
                            filterString = KeyboardInputAdmin("Filtre", nil, 25)
                        end
                            ...
                            for k,i in pairs(v.grade) do
                                RageUI.Button(v.name.." - ~c~["..k.."] "..i.label, nil, {RightLabel = ""}, true,function(h,a,s)
                                    if s then
                                        ...
                                        AtlantisRPEvent("AtlantisRPAdmin:setjob2",idjoueur, v.name, k)
                                        AtlantisRPEvent('AtlantisRPLog:Changerlegroup', IdSelected, v.name, k)
                                    end
                                end)
                            end
                        end
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'stafflist'), true, true, true, function()
                    for source, player in pairs(localPlayers) do
                        if player.rank ~= "user" then
                            if player.name == nil then
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'vehiclemain'), true, true, true, function()
                    ...
                    RageUI.Button("Réparer le moteur du véhicule", nil, {  }, true, function(h,a,s)
                        ...
                        if s then
                            ...
                            AtlantisRPEvent("SneakyLife:RepairMoteur", VehToNet(veh), ServerID)
                        end
                    end)
                    ...
                    RageUI.Button("Corriger les déformations du véhicule", nil, {  }, true, function(h,a,s)
                        ...
                        if s then
                            local veh = ESX.Game.GetClosestVehicle(GetEntityCoords(GetPlayerPed(-1)), nil)
                            local ServerID = GetPlayerServerId(NetworkGetEntityOwner(veh))
                            AtlantisRPEvent("SneakyLife:RepairDeformations", VehToNet(veh), ServerID)
                        end
                    end)
                    ...
                    RageUI.Button("Réparer le véhicule entièrement", nil, {  }, true, function(h,a,s)
                        ...
                        if s then
                            local veh = ESX.Game.GetClosestVehicle(GetEntityCoords(GetPlayerPed(-1)), nil)
                            local ServerID = GetPlayerServerId(NetworkGetEntityOwner(veh))
                            AtlantisRPEvent("SneakyLife:Repair", VehToNet(veh), ServerID)
                        end
                    end)
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'diversmain'), true, true, true, function()
                    ...
                    RageUI.Button("Clear la zone", nil, { RightLabel = "→" }, true, function(h,a,s)
                        if s then 
                            ...
                            AtlantisRPEvent("SneakyLife:ClearAreaFromObjects",GetEntityCoords(PlayerPedId()), players)
                        end
                    end)
                    RageUI.Button("Clear les props de la zone", nil, { RightLabel = "→" }, true, function(_,_,s)
                        if s then
                            ...
                            AtlantisRPEvent("DeleteEntityTable",props)
                        end
                    end)
...
AddEventHandler("core:envoyer", function(namestaff, msg)
	...
	ESX.ShowNotification('Message Staff', msg)
end)
...
RegisterNetEvent("AtlantisRPAdmin:Teleport")
AddEventHandler("AtlantisRPAdmin:Teleport", function(type, px, py, pz)
    if type == "goto" then
...
RegisterNetEvent("AtlantisRPAdmin:TeleportSpectate")
AddEventHandler("AtlantisRPAdmin:TeleportSpectate", function(coords)
    print(coords)
    local pos = 
    SetEntityCoords(PlayerPedId(), coords, -1, 0.0, 0.0, 0.0, true)
end)

RegisterNetEvent("AtlantisRPAdmin:RequestCoordsCamToServer")
AddEventHandler("AtlantisRPAdmin:RequestCoordsCamToServer", function()
