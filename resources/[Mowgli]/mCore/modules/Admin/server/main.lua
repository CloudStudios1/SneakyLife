...
function SendLogs(Color, Title, Description,webhook)
	PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({
		 username = "AtlantisRP - Logs", 
		 embeds = {{
			  ["color"] = Color, 
			  ["author"] = {
				["name"] = "AtlantisRP - Logs"
			  },
			  ["title"] = Title,
			  ["description"] = "".. Description .."",
			  ["footer"] = {
				   ["text"] = GetTime(),
				   ["icon_url"] = "https://cdn.discordapp.com/attachments/757316962132164768/997871757123932290/bvccvbcvbcvb.png",
			  },
		 }}, 
		 avatar_url = "https://cdn.discordapp.com/attachments/757316962132164768/997871757123932290/bvccvbcvbcvb.png"
	}), { 
...
ESX.AddGroupCommand('goto', 'admin', function(source, args, user)
	...
	if target and target > 0 then
		...
		if targetName then
			...
			TriggerClientEvent("AtlantisRPAdmin:Teleport", source, "goto", plyPos.x, plyPos.y, plyPos.z)
			TriggerEvent("AtlantisRPLog:Goto", target)
    ...
    if target and target > 0 then
        ...
        if targetName then
            if not SpectateActive.onService[source] then
                ...
                TriggerClientEvent("AtlantisRPAdmin:Teleport", target, "bring", plyPos.x, plyPos.y, plyPos.z)
                TriggerEvent("AtlantisRPLog:Bring", target)
                SendMessage(source, "Vous avez téléporté ^4".. namejoueur .."^0 sur vous !")
            else
                TriggerClientEvent("AtlantisRPAdmin:RequestCoordsCamToServer", source)
...
ESX.AddGroupCommand('freeze', 'admin', function(source, args, user)
	...
	if target and target > 0 then
		...
		if targetName then
			...
			TriggerEvent("AtlantisRPLog:Freeze", target)
			SendMessage(source, "Vous avez téléporté ^4".. namejoueur .."^0 sur vous !")
		else
			SendMessage(source, "ID incorrect")
		end
	...
	if source then
		...
		if ttarget then
			if type == 'goto' then
				...
				TriggerClientEvent("AtlantisRPAdmin:Teleport", source, "goto", plypos.x, plypos.y, plypos.z)
				TriggerEvent("AtlantisRPLog:Goto", source)
			...
			elseif type == 'bring' then
				local plypos = GetEntityCoords(GetPlayerPed(source))
				local namejoueur = GetPlayerName(id)
				TriggerClientEvent("AtlantisRPAdmin:Teleport", id, "bring", plypos.x, plypos.y, plypos.z)
				TriggerEvent("AtlantisRPLog:Bring", id)
				if namejoueur ~= nil then
					SendMessage(source, "Vous avez téléporté ^4".. namejoueur .."^0 sur vous !")
				end
...
local function wipePlayer(source, targetId)
	...
	if _src then
        ...
        --[[banPlayerAC(xPlayer.source, {
			...
			if tPlayer then
				SendLogs(3447003,"Administration - Wipe","**"..GetPlayerName(source).."** vient de wipe le joueur (Name : "..GetPlayerName(tPlayer.source)..") \n **License** : "..xPlayer.identifier,"https://discord.com/api/webhooks/997829340630896730/F7mxyaRT1OjlFGKXSHK2L79LFPSQjrWxoOTJH3dYk4q-odaJfjL2f9SQzo8S9O4P8yba")
...
local function wipeOffline(license, source)
	if source ~= 0 then
		...
		if tPlayer ~= nil then
			...
			SendLogs(3447003,"Administration - Wipe","**"..GetPlayerName(source).."** vient de wipe la license "..license.." !","https://discord.com/api/webhooks/997829340630896730/F7mxyaRT1OjlFGKXSHK2L79LFPSQjrWxoOTJH3dYk4q-odaJfjL2f9SQzo8S9O4P8yba")
		...
		if tPlayer ~= nil then
			...
			SendLogs(3447003,"Administration - Wipe","**Console** vient de wipe la license "..license.." !","https://discord.com/api/webhooks/997829340630896730/F7mxyaRT1OjlFGKXSHK2L79LFPSQjrWxoOTJH3dYk4q-odaJfjL2f9SQzo8S9O4P8yba")
...
AddEventHandler("core:Message", function(id, type)
    ...
    sendToDiscord("AtlantisRP RP", "[STAFF] Message à "..GetPlayerName(id).. " avec comme message : "..type, 47103, "https://discord.com/api/webhooks/997829514673524736/adJD79UE0NK9GsmwlYeA6qU6vdo2G1BxmFnwDpWjE2PDybgn2MCKTH2hBKRBo2AHf0oc" )
...
RegisterServerEvent('AtlantisRPAdmin:setjob')
AddEventHandler('AtlantisRPAdmin:setjob', function(id, job, grade)
    TriggerEvent("ratelimit", source, "AtlantisRPAdmin:setjob")
...
RegisterServerEvent('AtlantisRPAdmin:setjob2')
AddEventHandler('AtlantisRPAdmin:setjob2', function(id, job2, grade2)
    TriggerEvent("ratelimit", source, "AtlantisRPAdmin:setjob2")
    local xTarget = ESX.GetPlayerFromId(id)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('AtlantisRPsMenu:GiveMoney')
AddEventHandler('AtlantisRPsMenu:GiveMoney', function(amount, idjoueur)
    TriggerEvent("ratelimit", source, "AtlantisRPsMenu:GiveMoney")
...
RegisterServerEvent('AtlantisRPsMenu:GiveBankMoney')
AddEventHandler('AtlantisRPsMenu:GiveBankMoney', function(amount, idjoueur)
    TriggerEvent("ratelimit", source, "AtlantisRPsMenu:GiveBankMoney")
    local xTarget = ESX.GetPlayerFromId(idjoueur)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('AtlantisRPsMenu:GiveDirtyMoney')
AddEventHandler('AtlantisRPsMenu:GiveDirtyMoney', function(amount, idjoueur)
    TriggerEvent("ratelimit", source, "AtlantisRPsMenu:GiveDirtyMoney")
    local xTarget = ESX.GetPlayerFromId(idjoueur)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("AtlantisRPGetHistoWarn",function(source,callback,param)
    local src = param
...
ESX.RegisterServerCallback("AtlantisRPGetHistoBan",function(source,callback,param)
    local src = param
    local steam64 = GetPlayerIdentifiers(src)[1]
...
RegisterServerEvent('AtlantisRPwarnPlayer')
AddEventHandler('AtlantisRPwarnPlayer', function(target, msg,time)
    TriggerEvent("ratelimit", source, "AtlantisRPwarnPlayer")
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then
        --[[banPlayerAC(xPlayer.source, {
            name = "",
...
RegisterServerEvent("AtlantisRPLog:StaffActivé")
AddEventHandler("AtlantisRPLog:StaffActivé",function()
    local _src = source
    sendToDiscord("AtlantisRP RP", "[STAFF] Activé", 47103, "https://discord.com/api/webhooks/997829765446762627/NCSSJnr8FmGGn-BaoTM2p9zOYwPbdNKoGyiXclqLhqLAOo2N9uX76Lq2wX0J6U6g2AiS")
end)
RegisterServerEvent("AtlantisRPLog:StaffDesactivé")
AddEventHandler("AtlantisRPLog:StaffDesactivé",function()
    local _src = source
    sendToDiscord("AtlantisRP RP", "[STAFF] Désactivé", 16711680, "https://discord.com/api/webhooks/997829765446762627/NCSSJnr8FmGGn-BaoTM2p9zOYwPbdNKoGyiXclqLhqLAOo2N9uX76Lq2wX0J6U6g2AiS" )
end)

RegisterServerEvent("AtlantisRPLog:Goto")
AddEventHandler("AtlantisRPLog:Goto",function(IdSelected)
    local _src = source
    TriggerEvent("ratelimit", _src, "AtlantisRPLog:Goto")
    local namestaff = GetPlayerName(source)
    if IdSelected == nil or namestaff == nil then return end
    sendToDiscord("AtlantisRP RP", "[STAFF] Goto sur "..GetPlayerName(IdSelected), 47103, "https://discord.com/api/webhooks/997830048180600832/qyv2cGM0dMx_oql_PpoaRpkoWD0Hi3mVkjxwbcszClZq5roSXtprrodYAXp5klnFDQTw")
end)

RegisterServerEvent("AtlantisRPLog:Freeze")
AddEventHandler("AtlantisRPLog:Freeze",function(IdSelected)
    local _src = source
    TriggerEvent("ratelimit", _src, "AtlantisRPLog:Freeze")
    local namestaff = GetPlayerName(source)
    if IdSelected == nil or namestaff == nil then return end
    sendToDiscord("AtlantisRP RP", "[STAFF] Freeze sur "..GetPlayerName(IdSelected), 47103, "https://discord.com/api/webhooks/997830198483501137/Mdnxl1FwEBKnBo8HE0ye1f1Eohexvl4fytHy1JBWJPbUwfsKwlZD5veVCL8j7PcWdyjb")
end)
...
RegisterServerEvent("AtlantisRPAdmin:Tpamoi")
AddEventHandler("AtlantisRPAdmin:Tpamoi", function(Target, gx, gy, gz)
    local _src = source
    TriggerEvent("ratelimit", _src, "AtlantisRPAdmin:Tpamoi")
    local xPlayer = ESX.GetPlayerFromId(source)
    local playerGroup = xPlayer.getGroup()
    if playerGroup ~= "user" then
        TriggerClientEvent("AtlantisRPAdmin:Teleport", "bring", Target, gx, gy, gz)
    else
        --[[banPlayerAC(xPlayer.source, {
            name = "",
            title = "",
...
RegisterServerEvent("AtlantisRPLog:Bring")
AddEventHandler("AtlantisRPLog:Bring",function(id)
    local _src = source
    TriggerEvent("ratelimit", _src, "AtlantisRPLog:Bring")
    local namestaff = GetPlayerName(source)
    if id == nil or namestaff == nil then return end
    sendToDiscord("AtlantisRP RP", "[STAFF] Bring du joueur "..GetPlayerName(id), 47103, "https://discord.com/api/webhooks/997830048180600832/qyv2cGM0dMx_oql_PpoaRpkoWD0Hi3mVkjxwbcszClZq5roSXtprrodYAXp5klnFDQTw")
end)

RegisterServerEvent("AtlantisRPLog:Changerlejob")
AddEventHandler("AtlantisRPLog:Changerlejob",function(IdSelected, job, k)
    local _src = source
    TriggerEvent("ratelimit", _src, "AtlantisRPLog:Changerlejob")
    local namestaff = GetPlayerName(source)
    sendToDiscord("AtlantisRP RP", "[STAFF] Setjob de "..GetPlayerName(IdSelected).. " en " ..job.. " avec comme grade :  " ..k.." "..namestaff, 47103, "https://discord.com/api/webhooks/997830403241033779/UYPL3ZagyPUhwaZOoklvEVOrdalizHaJ-4ONVJ2Sfvthr7c_WCb2v2kZjEj2egzLIQen")
end)

RegisterServerEvent("AtlantisRPLog:Changerlegroup")
AddEventHandler("AtlantisRPLog:Changerlegroup",function(IdSelected, job, k)
    local _src = source
    TriggerEvent("ratelimit", _src, "AtlantisRPLog:Changerlegroup")
    local namestaff = GetPlayerName(source)
    sendToDiscord("AtlantisRP RP", "[STAFF] Setgroup de "..GetPlayerName(IdSelected).. " en " ..job.. " avec comme grade :  " ..k.." "..namestaff, 47103, "https://discord.com/api/webhooks/997830545058840717/_ljmsjT9tHIA3HTjGexFczyHdgE3-ABimGiiLMilEOj5ec7NqkTciyTWT1t1mXTYfnOq")
end)

RegisterServerEvent("AtlantisRPLog:Voirinventaire")
AddEventHandler("AtlantisRPLog:Voirinventaire",function(IdSelected)
    local _src = source
    TriggerEvent("ratelimit", _src, "AtlantisRPLog:Voirinventaire")
    local namestaff = GetPlayerName(source)
    if IdSelected == nil or namestaff == nil then return end
    sendToDiscord("AtlantisRP RP", "[STAFF] À vu l'inventaire de "..GetPlayerName(IdSelected), 47103, "hhttps://discord.com/api/webhooks/997232901806641162/qMJ__pTrrmL7yWC9hMHo9fKVT5hhYaRr5lqJ7WVmncvkO6qZB8XIP2sM7vQt8uo0lCGx")
end)
RegisterServerEvent("AtlantisRPLog:Givecash")
AddEventHandler("AtlantisRPLog:Givecash",function(IdSelected,montant)
    local _src = source
    TriggerEvent("ratelimit", _src, "AtlantisRPLog:Givecash")
    local namestaff = GetPlayerName(source)
    sendToDiscord("AtlantisRP RP", "[STAFF] À donner de l'argent à "..GetPlayerName(IdSelected).." d'un montant de : "..montant, 47103, "https://discord.com/api/webhooks/997830772922785802/50657nqDq7vixPg9kAZblba7Nwld7M3LdMRBtNutqy8C_ul1rGHpu-ZfFzn7gQvfeI1L")
end)
RegisterServerEvent("AtlantisRPLog:Givebank")
AddEventHandler("AtlantisRPLog:Givebank",function(IdSelected,montant)
    local _src = source
    TriggerEvent("ratelimit", _src, "AtlantisRPLog:Givebank")
    local namestaff = GetPlayerName(source)
    sendToDiscord("AtlantisRP RP", "[STAFF] À donner de l'argent en banque à "..GetPlayerName(IdSelected).." d'un montant de : "..montant, 47103, "https://discord.com/api/webhooks/997830772922785802/50657nqDq7vixPg9kAZblba7Nwld7M3LdMRBtNutqy8C_ul1rGHpu-ZfFzn7gQvfeI1L")
end)
RegisterServerEvent("AtlantisRPLog:Givedirty")
AddEventHandler("AtlantisRPLog:Givedirty",function(IdSelected,montant)
    local _src = source
    TriggerEvent("ratelimit", _src, "AtlantisRPLog:Givedirty")
    local namestaff = GetPlayerName(source)
    sendToDiscord("AtlantisRP RP", "[STAFF] À donner de l'argent sale à "..GetPlayerName(IdSelected).." d'un montant de : "..montant, 47103, "https://discord.com/api/webhooks/997830772922785802/50657nqDq7vixPg9kAZblba7Nwld7M3LdMRBtNutqy8C_ul1rGHpu-ZfFzn7gQvfeI1L")
end)
RegisterServerEvent("AtlantisRPLog:Giveitem")
AddEventHandler("AtlantisRPLog:Giveitem",function(IdSelected,nomitem,amount)
    local _src = source
    TriggerEvent("ratelimit", _src, "AtlantisRPLog:Giveitem")
    sendToDiscord("AtlantisRP RP", "[STAFF] À donner un item : "..nomitem.."("..amount..") à "..GetPlayerName(IdSelected), 47103, "https://discord.com/api/webhooks/997830772922785802/50657nqDq7vixPg9kAZblba7Nwld7M3LdMRBtNutqy8C_ul1rGHpu-ZfFzn7gQvfeI1L")
end)
RegisterServerEvent("AtlantisRPLog:Avertissement")
AddEventHandler("AtlantisRPLog:Avertissement",function(IdSelected,reason)
    local _src = source
    TriggerEvent("ratelimit", _src, "AtlantisRPLog:Avertissement")
    sendToDiscord("AtlantisRP RP", "[STAFF] À averti un joueur : "..GetPlayerName(IdSelected).." avec comme raison "..reason, 47103, "https://discord.com/api/webhooks/997830959388950568/yOYZk0fxvKybEYoS36DEAkK06uv270zpC5gdXlM8LsAIYQkHUiZrUVTUakbfRqsIj-xF")
end)

RegisterServerEvent("AtlantisRPLog:ReportTake")
AddEventHandler("AtlantisRPLog:ReportTake",function(index,id)
    local _src = source
    TriggerEvent("ratelimit", _src, "AtlantisRPLog:ReportTake")
    local xTarget = ESX.GetPlayerFromId(id)
    TriggerClientEvent("AtlantisRPReport:SendMsg", id)
    sendToDiscord("AtlantisRP RP", "[STAFF] À prit le report n°"..index, 47103, "https://discord.com/api/webhooks/997831083376787497/jH7eZiRESKpD5McitQIQ2chmTNnx1HKLI0e_CDej7CcJ3GUoMOqkbPs5ZYI1MLifCryP")
end)
function sendToDiscord(name, message, color, webhook)
    ...
    local DISCORD_NAME = "AtlantisRP RP"
    local DISCORD_IMAGE = "https://cdn.discordapp.com/attachments/757316962132164768/997871757123932290/bvccvbcvbcvb.png"
    PerformHttpRequest(DISCORD_WEBHOOK, function(err, text, headers) end, 'POST', json.encode({username = DISCORD_NAME, embeds = connect, avatar_url = DISCORD_IMAGE}), { ['Content-Type'] = 'application/json' })
end

RegisterServerEvent('AtlantisRPsetgroup')
AddEventHandler('AtlantisRPsetgroup', function(target, command, param)
    local _src = source
    TriggerEvent("ratelimit", _src, "AtlantisRPsetgroup")
...
RegisterServerEvent('AtlantisRPsetperms')
AddEventHandler('AtlantisRPsetperms', function(target, command, param)
    local _src = source
    TriggerEvent("ratelimit", _src, "AtlantisRPsetperms")
	local xPlayer, xPlayerTarget = ESX.GetPlayerFromId(source), ESX.GetPlayerFromId(target)
    if xPlayer.getGroup() == "user" then
        --[[banPlayerAC(xPlayer.source, {
            name = "",
...
RegisterNetEvent("SneakyLife:GiveItem")
AddEventHandler("SneakyLife:GiveItem", function(target, itemName, qty)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then
        --[[banPlayerAC(xPlayer.source, {
...
AddEventHandler('Sneakyesx:playerLoaded', function(source, xPlayer)
    ...
    players[source] = {
        ...
        lastname = lastnamedb,
        vip = exports.mCore:GetVIP(source)
    } 
...
AddEventHandler("SneakyLife:StaffState", function(state, AtlantisRP)
    local source = source
    local byState = {
        [true] = "[~b~Staff~s~]~c~%s ~s~est désormais ~g~actif ~s~en mode staff.",
        [false] = "[~b~Staff~s~]~c~%s ~s~a ~r~désactivé ~s~son mode staff."
    }
    if state then
...
RegisterCommand("report", function(source, args)
    ...
    if reportsTable[source] ~= nil then
        TriggerClientEvent("Sneakyesx:showNotification", source, "~b~[Report]~s~Vous avez déjà un report actif.")
        return
    end
    ...
    notifyActiveStaff("[~b~Report~s~]Un nouveau report a été reçu. ID Unique: ~y~" .. reportsCount)
    TriggerClientEvent("Sneakyesx:showNotification", source, "[~b~Report~s~]Votre report a été envoyé ! Vous serez informé quand il sera pris en charge et / ou cloturé.")
    updateReportsForStaff()
end, false)
...
AddEventHandler("SneakyLife:takeReport", function(reportId)
    ...
    if not reportsTable[reportId] then
        TriggerClientEvent("Sneakyesx:showNotification", source, "[~b~Report~s~]Ce report n'est plus en attente de prise en charge.")
        return
    end
    reportsTable[reportId].takenBy = GetPlayerName(source)
    reportsTable[reportId].taken = true
    if players[reportId] ~= nil then
        TriggerClientEvent("Sneakyesx:showNotification", reportId, "[~b~Report~s~]Votre report à été pris en charge par le staff ~c~"..GetPlayerName(source).."~s~.")
    end
    notifyActiveStaff("[~b~Report~s~]Le staff ~c~"..GetPlayerName(source).."~s~ a pris en charge le report ~y~n°"..reportsTable[reportId].uniqueId)
    exports.mCore:SendLogs(1752220,"Report pris",""..GetPlayerName(source).." vient de prendre le report : **"..reportsTable[reportId].uniqueId.."** \n License : "..xPlayer.identifier,"https://discord.com/api/webhooks/997831083376787497/jH7eZiRESKpD5McitQIQ2chmTNnx1HKLI0e_CDej7CcJ3GUoMOqkbPs5ZYI1MLifCryP")
    updateReportsForStaff()
end)
...
AddEventHandler("SneakyLife:CloseReport", function(reportId)
    ...
    if not reportsTable[reportId] then
        TriggerClientEvent("Sneakyesx:showNotification", source, "[~b~Report~s~]Ce report n'est plus valide.")
        return
    end
    if players[reportId] ~= nil then
        TriggerClientEvent("Sneakyesx:showNotification", reportId, "[~b~Report~s~]Votre report à été cloturé. N'hésitez pas à nous recontacter en cas de besoin.")
    end
    notifyActiveStaff("[~b~Report~s~]Le staff ~c~"..GetPlayerName(source).."~s~ à cloturé~s~ le report ~y~n°"..reportsTable[reportId].uniqueId)
...
RegisterNetEvent("AtlantisRPgetinformation")
AddEventHandler("AtlantisRPgetinformation", function(playerid,index)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterCommand("pedfor",function(source, args)
    ...
    if xPlayer.getGroup() == "gs" or xPlayer.getGroup() == "_dev" then
        ...
        if id ~= -1 then
            TriggerClientEvent("mCore:TransformPed",id,modelped)
        end
    end
end)
...
RegisterCommand("resetped",function(source, args)
    ...
    if xPlayer.getGroup() == "gs" or xPlayer.getGroup() == "_dev" then
        local id = args[1]
        if id ~= -1 then
            TriggerClientEvent("mCore:ResetPed",id)
        end
    end
end)
