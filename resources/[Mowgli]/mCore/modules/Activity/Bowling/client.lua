...
Citizen.CreateThread(function()
    ...
    Wait(2500)
    for k, v in pairs(Bowling['Bowling']['BowlingBalls']) do
    ...
    while true do
        ...
        if not bowling then
            local distance = GetDistanceBetweenCoords(GetEntityCoords(me), Bowling['Bowling']['GetBall'], true)
            if distance <= 15.0 then
                sleep = 0
                DrawMarker(0, Bowling['Bowling']['GetBall'], vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(0.5, 0.5, 0.4), 35, 255, 75, 255)
...
AddEventHandler('loaf_bowling:chooseTrack', function(occupiedTracks)
    ...
    for i = 1, #Bowling['Bowling']['Tracks'] do
        if not occupiedTracks[i] then table.insert(tracksLeft, Bowling['Bowling']['Tracks'][i]) end
    end 
    if #tracksLeft >= 1 then
        local cam = CreateCam('DEFAULT_SCRIPTED_Camera', 1)
        SetCamCoord(cam, Bowling['Bowling']['Cam']['Start'])
        RenderScriptCams(1, 0, 0, 1, 1)
        PointCamAtCoord(cam, Bowling['Bowling']['Cam']['End'])
        SetCamFov(cam, Bowling['Bowling']['Cam']['Fov'])
        while true do
            ...
            if IsControlJustReleased(0, 215) then
                for i = 1, #Bowling['Bowling']['Tracks'] do
                    if Bowling['Bowling']['Tracks'][i] == tracksLeft[currentTrack] then
...
AddEventHandler('loaf_bowling:play', function(chosenTrack)
    ...
    local track, me, currentCam, pins, force, secondTry = Bowling['Bowling']['Tracks'][chosenTrack], PlayerPedId(), 1, {}, 0.0, false
    SetEntityCoords(me, track)
    SetEntityHeading(me, track.w)

    for k, v in pairs(Bowling['Bowling']['BowlingPinSets'][chosenTrack]) do
    ...
    while bowling do
        ...
        if IsDisabledControlPressed(0, 174) then
            if GetEntityHeading(me)+0.15 <= Bowling['Bowling']['Heading'][2] then
                SetEntityHeading(me, GetEntityHeading(me)+0.15)
            end
        elseif IsDisabledControlPressed(0, 175) then
            if GetEntityHeading(me)-0.15 >= Bowling['Bowling']['Heading'][1] then
                SetEntityHeading(me, GetEntityHeading(me)-0.15)
            end
        ...
        elseif IsDisabledControlJustReleased(0, 24) then
            ...
            if pinsDown == #Bowling['Bowling']['BowlingPinSets'][chosenTrack] then
                if secondTry then
                    secondTry = false
                ...
                if secondTry then
                    wastedMsg(Strings['x_pinsdown'][1], (Strings['x_pinsdown'][2]):format(pinsDown, #Bowling['Bowling']['BowlingPinSets'][chosenTrack]), 2000)
                    secondTry = false
                else
                    secondTry = true
                end
