...
function openMenu(chasse)
    if openedMenu then
        ...
        Citizen.CreateThread(function()
            ...
            while openedMenu do
                Wait(1.0)
                RageUI.IsVisible(chasseMenu, true, true, true, function()
                    ...
                    if hasChasse == nil then
                        ...
                        if not hasChasse then
                            RageUI.Button("Commencer la chasse", nil, {RightLabel = "→→"}, true, function(h, a ,s)
                                if s then
                                    AtlantisRPEvent("sChasse:start", GetEntityCoords(PlayerPedId()), chasse)
                                end
                            end)
                        else
                            RageUI.Button("Arrêter la chasse", nil, {RightLabel = "→→"}, true, function(h, a ,s)
                                if s then
                                    if not exports.inventaire:GetFastWeaponsChasse() then
                                        AtlantisRPEvent("sChasse:stop", true, GetEntityCoords(PlayerPedId()), chasse)
                                    else
