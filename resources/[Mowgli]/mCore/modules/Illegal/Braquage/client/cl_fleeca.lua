...
function Fleeca:StartScalform()
    ...
    AtlantisRPEvent("clp:removeitem", "drill", 0)
...
function Fleeca:OpenDoorFl(id)
    ...
    if getObjdoor and DoesEntityExist(getObjdoor) then
        ...
        AtlantisRPEvent("cFleeca:OpenDoor", getObjdoor, doorpos)
    end
end
...
AddEventHandler("cFleeca:BinginDrill", function()
    ...
    AtlantisRPEvent("cFleeca:SetCooldown", Fleeca.nFleeca.id)
end)
...
AddEventHandler("cFleeca:clientBankHeist", function(bool, _)
    if bool == 1 then
        Fleeca.nFleeca = _;
        AtlantisRPEvent("cFleeca:GetCoolDown", Fleeca.nFleeca.id)
    elseif bool == 2 then
        if _ then
    ...
    elseif bool == 3 then
        ...
        AtlantisRPEvent("cFleeca:GiveMoney", moNeoe)
        ESX.ShowNotification("Vous avez récupéré "..moNeoe.."~g~$~s~.")
        Wait(15*60*1000)
        if Fleeca.ObjectDoorid ~= nil and Fleeca.ObjectDooridPos ~= nil then 
            AtlantisRPEvent("cFleeca:CloseDoor", Fleeca.ObjectDoorid, Fleeca.ObjectDooridPos)
        end
    end
end)