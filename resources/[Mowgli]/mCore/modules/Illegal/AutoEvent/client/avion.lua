...
function avionEvent(data, zone)
    ...
    Citizen.CreateThread(function()
        ...
        ESX.ShowAdvancedNotification("Joaquìn Hernàndez", "~r~Event illégal", data.message, "CHAR_ORTEGA",1)
        ...
        if not EventStop then
            ...
            while ArmesRecup < 10 do
                ...
                if not EventStop then
                    ...
                    AtlantisRPEvent("RS_AUTOEVENT:GetItem", item, nombre)
                    if EventStop then return end
                    if EventStop then break end
                end

                if ArmesRecup >= 10 then
                    ESX.ShowAdvancedNotification("Joaquìn Hernàndez", "~r~Event illégal", "Tu as terminé le travail avant les autres ! On va leur laisser un peu de temps !", "CHAR_ORTEGA",1)
                    Wait(10000)
                    for k,v in pairs(ObjTable) do
                        RemoveObj(v)
                    end
                    AtlantisRPEvent("RS_AUTOEVENT:Recuperer")
                    sended = true
                    break
                end
                if EventStop then 
            ...
            ESX.ShowAdvancedNotification("Joaquìn Hernàndez", "~r~Event illégal", "Cargaison récupérée!", "CHAR_ORTEGA",1)
