...
openedBuyMenu = false
local buyMenu = RageUI.CreateMenu("", "KadirTheLol", 80, 95, "root_cause", "sneakylife")
    ...
    if openedBuyMenu then
        ...
        Citizen.CreateThread(function()
            ...
            while openedBuyMenu do
                Wait(1.0)
                RageUI.IsVisible(buyMenu, true, true, true, function()
                    ...
                    RageUI.Button("Visiter", nil, {RightLabel = "~y~Entrer~s~ →"}, true, function(_, _, Select)
                        if (Select) then
                            AtlantisRPEvent("sLaboratoire:Visite", GetEntityCoords(PlayerPedId()), table)
                        end
                    end)
                    RageUI.Button("Acheter", "Prix "..table.price.."~g~$~s~", {RightLabel = "~g~Oui~s~ →"}, true, function(_, _, Select)
                        if (Select) then
                            AtlantisRPEvent("sLaboratoire:buyLab", table, ESX.PlayerData.job2.name, ESX.PlayerData.job2.label)
                        end
                    end)
                end)
            end
    ...
    if openedVisiteMenu then
        ...
        Citizen.CreateThread(function()
            while openedVisiteMenu do
                Wait(1.0)
                RageUI.IsVisible(visiteMenu, true, true, true, function()
                    ...
                    RageUI.Button("~r~Quitter le mode visite~s~", nil, {RightLabel = "→"}, true, function(_, _, Select)
                        if (Select) then
                            if openedVisiteMenu then
                                AtlantisRPEvent("sLaboratoire:Exit")
                                openedVisiteMenu = false
                                FreezeEntityPosition(PlayerPedId(), false)
                                RageUI.Visible(visiteMenu, false)
                                openBuyMenu(table)
