...
local createdMenu = RageUI.CreateMenu("", "Laboratoire", 80, 95, "root_cause", "sneakylife")
    ...
    if openedCreatedMenu then
        ...
        Citizen.CreateThread(function()
            while openedCreatedMenu do
                Wait(1.0)
                RageUI.IsVisible(createdMenu, true, true, true, function()
                ...
                RageUI.IsVisible(createdSubMenu, true, true, true, function()
                ...
                RageUI.IsVisible(createdViewMenu, true, true, true, function()
                    ...
                    if selectLab.owner ~= nil then
                        RageUI.Button("Dissocier le laboratoire des ~b~"..selectLab.ownerName, "~r~Attention~s~ : Pas de retour possible !", {RightLabel = "~g~Oui~s~ →"}, true, function(_, _, Select)
                            if (Select) then 
                                AtlantisRPEvent("sLaboratoire:removeOwner", selectLab, selectLab.owner, selectLab.ownerName)
                                RageUI.GoBack()
                            end
                        end)    
                    end
