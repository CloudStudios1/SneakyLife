                ...
                if IsControlJustPressed(0, 51) then 
                    if v.itemrequired then
                            ...
                            if count >= v.itemcountrequired then 
                                if v.itemrequired2 then 
                                    ESX.TriggerServerCallback("sDrugs:getItemAmount", function(count)
                                        if count >= v.itemcountrequired2 then
                                            ...
                                            if not IsPedSittingInAnyVehicle(PlayerPedId()) and PedHealth >= 10 and not ProgressBarExists() then
                                                ...
                                                RemoveProgressBar()
                                                TriggerServerEvent("mCore:FarmLabo", v.labocolor, v.itemwin, v.itemcountwin, v.itemrequired, v.itemcountrequired, v.itemrequired2, v.itemcountrequired2)
                                                Wait(1500)
                                            end
                                        else
                                            ESX.ShowNotification("~r~Vous n'avez assez d'item demandé pour commencer à traiter.")
                                        end
                                    end,v.itemrequired2)
                                else
                                    local PedHealth = GetEntityHealth(PlayerPedId())
                                    if not IsPedSittingInAnyVehicle(PlayerPedId()) and PedHealth >= 10 and not ProgressBarExists() then
                                        ...
                                        RemoveProgressBar()
                                        TriggerServerEvent("mCore:FarmLabo", v.labocolor, v.itemwin, v.itemcountwin, v.itemrequired, v.itemcountrequired, nil, nil)
                                        Wait(1500)
                                    end
                                end
                            else
                        ...
                        if not IsPedSittingInAnyVehicle(PlayerPedId()) and PedHealth >= 10 and not ProgressBarExists() then
                            ...
                            RemoveProgressBar()
                            TriggerServerEvent("mCore:FarmLabo", v.labocolor, v.itemwin, v.itemcountwin, nil, nil, nil, nil)
                            Wait(1500)
                        end
                    end
                end
...
function finalDrugs()
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            Citizen.Wait(14500)
            ClearPedTasksImmediately(PlayerPedId())
            AtlantisRPEvent("sDrugs:Removeitem")
            cooldowndrugs = false
            onFarming = false
            break
        end
