			...
			Wait(1)
						...
						if s then
							if IsPedInAnyVehicle(PlayerPedId(), -1) then
				            	...
				            	if closestPlayer ~= -1 and closestDistance <= 3.0 then
									...
									AtlantisRPEvent('sIllegal:Arrestation', "arrest", GetPlayerServerId(target), GetEntityHeading(PlayerPedId()), GetEntityCoords(PlayerPedId()), GetEntityForwardVector(PlayerPedId()))
								else
									ESX.ShowNotification("~r~Personne à proximité")
								end
							end
					...
					RageUI.Button("Libérer", nil, {RightLabel = "→"}, closestPlayer ~= -1 and closestDistance <= 3.0, function(_, a, s)
						...
						if s then
							if IsPedInAnyVehicle(PlayerPedId(), -1) then
				            	...
				            	if closestPlayer ~= -1 and closestDistance <= 3.0 then
									local target, _ = ESX.Game.GetClosestPlayer()
									AtlantisRPEvent('sIllegal:Arrestation', "notarrest", GetPlayerServerId(target), GetEntityHeading(PlayerPedId()), GetEntityCoords(PlayerPedId()), GetEntityForwardVector(PlayerPedId()))
								else
									ESX.ShowNotification("~r~Personne à proximité")
								end
							end
				...
				RageUI.IsVisible(RMenu:Get("secondmenu",'groupe'),true,true,true,function()
					RageUI.Button("Recruter", nil, {RightLabel = "→"}, closestPlayer ~= -1 and closestDistance <= 3.0, function(_, a, s)
						...
						if s then
							TriggerServerEvent('SneakyLife:RecruitPlayer', GetPlayerServerId(closestPlayer), ESX.PlayerData.job2.name, 0)
						end
					end)
					RageUI.Button("Promouvoir", nil, {RightLabel = "→"}, closestPlayer ~= -1 and closestDistance <= 3.0, function(_, a, s)
						...
						if s then
							TriggerServerEvent('SneakyLife:PromotePlayer', GetPlayerServerId(closestPlayer))
						end
					end)
					RageUI.Button("Rétrograder", nil, {RightLabel = "→"}, closestPlayer ~= -1 and closestDistance <= 3.0, function(_, a, s)
						...
						if s then
							TriggerServerEvent('SneakyLife:RetrogradePlayer', GetPlayerServerId(closestPlayer))
						end
					end)
					RageUI.Button("Virer", nil, {RightLabel = "→"}, closestPlayer ~= -1 and closestDistance <= 3.0, function(_, a, s)
						...
						if s then
							TriggerServerEvent('SneakyLife:VirerPlayer', GetPlayerServerId(closestPlayer))
						end
					end)
				end, function() end)
