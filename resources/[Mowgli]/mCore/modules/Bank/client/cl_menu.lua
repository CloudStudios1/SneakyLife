...
function CheckSolde()
    AtlantisRPEvent("SneakyLife:solde",action)
end
...
function Deposit()
    ...
    if amount ~= nil then
        ...
        if type(amount) == 'number' then
            AtlantisRPEvent('SneakyLife:deposit', amount)
            succes = 1
        else
...
function Withdraw()
    ...
    if amount ~= nil then
        ...
        if type(amount) == 'number' then
            AtlantisRPEvent('SneakyLife:withdraw', amount)
            succes = 1
        else
            ESX.ShowNotification("~r~Attention~s~\nVous n'avez pas saisi de montant")
        end
...
function OpenbanqueRageUIMenu()
    ...
    if Bank.Menu then
        ...
        Citizen.CreateThread(function()
			while Bank.Menu do
                RageUI.IsVisible(RMenu:Get('banque', 'main'), true, true, true, function()
                    if zone == 1 then
