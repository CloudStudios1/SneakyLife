    ...
    MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE plate = @plate", {
        ...
        if result[1] ~= nil then
            if result[1].owner ~= xPlayer.identifier then
                TriggerClientEvent("Sneakyesx:showNotification", xPlayer.source, "~r~Erreur~s~Vous ne pouvez pas stocker ce vehicle !")
                loaded = false
            else
            ...
            if #sGarageManager.list[garageId].vehicles > v.max then
                for k,v in pairs(sGarageManager.list[garageId].vehicles) do
                    if v.plate == plate then
                        ...
                        return TriggerClientEvent("Sneakyesx:showNotification", source, "~r~Erreur~s~Il n'y a plus assez de ~b~places~s~ dans le garage !")
                    end
                end
            end
        end
