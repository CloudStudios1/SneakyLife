	...
	MySQL.Async.fetchAll("SELECT * FROM vips WHERE identifier=@identifier",{
        ...
        if data[1] then
            if tonumber(data[1].expiration) <= os.time() then
                xPlayer.showNotification('Votre VIP a  expirer')
                ...
                xPlayer.showNotification('Votre VIP :\nPrendra fin dans ~b~'..math.floor(day).. ' ~w~jours et ~b~'..txthrs.."~w~ heure(s).")
                if not VipTable[xPlayer.identifier] then
                    VipTable[xPlayer.identifier] = true
                end
                TriggerClientEvent('Mowgli:updateVIP', src, true)
            end
        else
            VipTable[xPlayer.identifier] = false
            TriggerClientEvent('Mowgli:updateVIP', src, false)
        end
	end)
end)

RegisterNetEvent('AtlantisRP:GetVIP')
AddEventHandler('AtlantisRP:GetVIP',function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if source == 0 then
        if args[1] then
            ...
            if not VipTable[TARGET.identifier] then
                ...
                local jour = tonumber(args[2])
                local expiration = (jour * 86400)
                ...
                TARGET.showNotification('Vous venez d\'obtenir un VIP')
                VipTable[TARGET.identifier] = true
                TriggerClientEvent('Mowgli:updateVIP', TARGET.source, true)
            end
        end
    end
end)
