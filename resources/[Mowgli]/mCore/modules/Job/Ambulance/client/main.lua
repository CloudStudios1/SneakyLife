	...
	if AmbulanceRdv.Menu then
        ...
        Citizen.CreateThread(function()
			while AmbulanceRdv.Menu do
                RageUI.IsVisible(RMenu:Get('ambulancerdv', 'main'), true, true, true, function()
					if AppelerCooldown == false then
						RageUI.Button("Appeler un médecin", false, {RightLabel = "~g~→ Sonner"}, not AppelerCooldown, function(h,a,s)
							if s then
								AtlantisRPEvent("Ems:DemandeRdv","Une personne demande un médecin.")
	...
	if AmbulancePharmacie.Menu then
        ...
        Citizen.CreateThread(function()
			while AmbulancePharmacie.Menu do
                RageUI.IsVisible(RMenu:Get('ambulancepharmacie', 'main'), true, true, true, function()
                    RageUI.Button("Medikit", false, {RightLabel = "~g~→ Prendre"}, true, function(h,a,s)
						if s then
							AtlantisRPEvent('Sneakyesx_ambulancejob:giveItem', "medikit")
						end
                    end)
					RageUI.Button("Bandage", false, {RightLabel = "~g~→ Prendre"}, true, function(h,a,s)
						if s then
							AtlantisRPEvent('Sneakyesx_ambulancejob:giveItem', "bandage")
						end
                    end)
					RageUI.Button("Perfusion", false, {RightLabel = "~g~→ Prendre"}, true, function(h,a,s)
						if s then
							AtlantisRPEvent('Sneakyesx_ambulancejob:giveItem', "perfusion")
						end
                    end)
					RageUI.Button("Fauteuil Roulant", false, {RightLabel = "~g~→ Prendre"}, true, function(h,a,s)
						if s then
							AtlantisRPEvent('Sneakyesx_ambulancejob:giveItem', "wheelchair")
						end
                    end)
					RageUI.Button("Lit", false, {RightLabel = "~g~→ Prendre"}, true, function(h,a,s)
						if s then
							AtlantisRPEvent('Sneakyesx_ambulancejob:giveItem', "bed")
						end
                    end)
                end)
				Wait(0)
    ...
    if AmbulanceActionMenu.Menu then 
        ...
        Citizen.CreateThread(function()
			while AmbulanceActionMenu.Menu do
                RageUI.IsVisible(RMenu:Get('ambulanceactionmenu', 'main'), true, true, true, function()
                    ESX.PlayerData = ESX.GetPlayerData()
									...
									if qtty > 0 then
										...
										ESX.TriggerServerCallback("EMS:PlayerIsDead", function(dead)
											...
											if dead then
												...
												AtlantisRPEvent("EMS:RevivePLayer", GetPlayerServerId(closestPlayer))
												AtlantisRPEvent('Sneakyesx_ambulancejob:removeItem', 'medikit')
												IsBusy = false
								...
								ESX.TriggerServerCallback('Sneakyesx_ambulancejob:getItemAmount', function(qtty)
									if qtty > 0 then
										...
										if health > 0 then
											...
											AtlantisRPEvent('Sneakyesx_ambulancejob:removeItem', 'bandage')
											AtlantisRPEvent('Sneakyesx_ambulancejob:heal', GetPlayerServerId(closestPlayer), 'small')
											IsBusy = false
										else
								...
								ESX.TriggerServerCallback('Sneakyesx_ambulancejob:getItemAmount', function(qtty)
									if qtty > 0 then
										...
										if health > 0 then
											...
											AtlantisRPEvent('Sneakyesx_ambulancejob:removeItem', 'medikit')
											AtlantisRPEvent('Sneakyesx_ambulancejob:heal', GetPlayerServerId(closestPlayer), 'big')
											IsBusy = false
										else
											ESX.ShowAdvancedNotification('Ambulance', 'Message', 'Cette personne est inconsciente!', 'CHAR_CALL911', 1)
										end
