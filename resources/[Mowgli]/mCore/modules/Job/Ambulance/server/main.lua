...
RegisterServerEvent('SneakyLife:DeadPlayer')
AddEventHandler('SneakyLife:DeadPlayer', function()
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('SneakyLife:StopDeadPlayer')
AddEventHandler('SneakyLife:StopDeadPlayer', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	MySQL.Async.execute('UPDATE users SET dead = @dead WHERE identifier = @identifier', {
		['@identifier'] = xPlayer.identifier,
		['@dead'] = 0
...
ESX.RegisterUsableItem('wheelchair', function(source)
	...
	TriggerClientEvent('mCore:useChair', _source)
end)
...
ESX.RegisterUsableItem('bed', function(source)
	...
	TriggerClientEvent('mCore:useBed', _source)
end)

RegisterNetEvent('mCore:wheelchair')
AddEventHandler('mCore:wheelchair', function(item_name)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.job.name ~= "ambulance" then
        --[[banPlayerAC(xPlayer.source, {
            name = "",
...
RegisterNetEvent('mCore:bedSystem')
AddEventHandler('mCore:bedSystem', function(item_name)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.job.name ~= "ambulance" then
        --[[banPlayerAC(xPlayer.source, {
            name = "",
