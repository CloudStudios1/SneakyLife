                ...
                if not callcooldown then
                    RageUI.Button("Appeler la LSPD", nil, {RightLabel = "→"}, onService and not callcooldown , function(h, a, s)
                        if s then
                            ...
                            AtlantisRPEvent("sPolice:enterAppel", pos.x, pos.y, pos.z, "Braquage", "Notre LTD se fait actuellement braquer !")
    ...
    if GarageLtdSud.Menu then
        ...
        Citizen.CreateThread(function()
            while GarageLtdSud.Menu do
                RageUI.IsVisible(RMenu:Get('garageltd_sud', 'main'), true, true, true, function()
