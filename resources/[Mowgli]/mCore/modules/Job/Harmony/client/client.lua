    ...
    if HarmonyVestiaire.Menu then
        ...
        Citizen.CreateThread(function()
            while HarmonyVestiaire.Menu do
                RageUI.IsVisible(RMenu:Get('harmonyvestiaire', 'main'), true, true, true, function()
	...
	FreezeEntityPosition(PlayerPedId(), true)
        ...
        Citizen.CreateThread(function()
			while MecanoKitRepa.Menu do
                RageUI.IsVisible(RMenu:Get('harmonyshop', 'main'), true, true, true, function()
                    RageUI.Button("Kit de réparation", false, {RightLabel = "→ Acheter 500~g~$"}, true, function(h,a,s)
					if s then
						AtlantisRPEvent('AtlantisRPharmony:giveItem', "fixkit",500)
					end
                    end)
                end)
				Wait(0)
    ...
    if HarmonyGarage.Menu then
        ...
        Citizen.CreateThread(function()
            while HarmonyGarage.Menu do
                RageUI.IsVisible(RMenu:Get('harmonygarage', 'main'), true, true, true, function()
    ...
    if HarmonyActionMenu.Menu then 
        ...
        Citizen.CreateThread(function()
			while HarmonyActionMenu.Menu do
                RageUI.IsVisible(RMenu:Get('harmonyactionmenu', 'main'), true, true, true, function()
				RageUI.List("Annonce", ActionsAnnonceHarmony, ActionsAnnonceHarmonyIndex, nil, {}, true, function(Hovered, Active, Selected, Index) 
					if (Selected) then 
						if Index == 1 then
							...
							AtlantisRPEvent('harmony:addAnnounce', announce)
						elseif Index == 2 then
...
AddEventHandler('harmony:targetAnnounce', function(announce)
    if announce == 'ouvert' then
        ESX.ShowAdvancedNotification("Harmony Repair's and Custom's", '~y~Informations', "- Mécano ~g~ouvert~s~- Horaire : ~g~Maintenant", "CHAR_HARMONY", 1)
    elseif announce == 'fermeture' then
        ESX.ShowAdvancedNotification("Harmony Repair's and Custom's", '~y~Informations', "- Mécano ~r~fermé~s~- Horaire : ~g~Maintenant", "CHAR_HARMONY", 1)
    end
end)