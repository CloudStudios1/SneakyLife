    ...
    if MecanoVestiaire.Menu then
        ...
        Citizen.CreateThread(function()
            while MecanoVestiaire.Menu do
                RageUI.IsVisible(RMenu:Get('mecanovestiaire', 'main'), true, true, true, function()
	...
	FreezeEntityPosition(PlayerPedId(), true)
        ...
        Citizen.CreateThread(function()
			while MecanoKitRepa.Menu do
                RageUI.IsVisible(RMenu:Get('mecanoshop', 'main'), true, true, true, function()
                    RageUI.Button("Kit de réparation", false, {RightLabel = "→ Acheter 500~g~$"}, true, function(h,a,s)
					if s then
						AtlantisRPEvent('AtlantisRPmecano:giveItem', "fixkit",500)
					end
                    end)
                end)
				Wait(0)
    ...
    if MecanoGarage.Menu then
        ...
        Citizen.CreateThread(function()
            while MecanoGarage.Menu do
                RageUI.IsVisible(RMenu:Get('mecanogarage', 'main'), true, true, true, function()
    ...
    if MecanoActionMenu.Menu then 
        ...
        Citizen.CreateThread(function()
			while MecanoActionMenu.Menu do
                RageUI.IsVisible(RMenu:Get('mecanoactionmenu', 'main'), true, true, true, function()
				RageUI.List("Annonce", ActionsAnnonce, ActionsAnnonceIndex, nil, {}, true, function(Hovered, Active, Selected, Index) 
					if (Selected) then 
						if Index == 1 then
							...
							AtlantisRPEvent('mecano:addAnnounce', announce)
						elseif Index == 2 then
...
AddEventHandler('mecano:targetAnnounce', function(announce)
    if announce == 'ouvert' then
        ESX.ShowAdvancedNotification('Mécano', '~y~Informations', "- Mécano ~g~ouvert~s~- Horaire : ~g~Maintenant", "CHAR_BENNYS", 1)
    elseif announce == 'fermeture' then
        ESX.ShowAdvancedNotification('Mécano', '~y~Informations', "- Mécano ~r~fermé~s~- Horaire : ~g~Maintenant", "CHAR_BENNYS", 1)
    end
end)
