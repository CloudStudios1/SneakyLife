...
AddEventHandler('announce:unicorn', function(announce)
    if announce == 'ouvert' then
        ESX.ShowAdvancedNotification('Unicorn', '~y~Informations', "- Unicorn ~g~ouvert~s~- Horaire : ~g~Maintenant", "CHAR_UNICORN", 1)
    elseif announce == 'fermeture' then
        ESX.ShowAdvancedNotification('Unicorn', '~y~Informations', "- Unicorn ~r~fermé~s~- Horaire : ~g~Maintenant", "CHAR_UNICORN", 1)
    end
end)
    ...
    if MenuUnicorn.Menu then
        ...
        Citizen.CreateThread(function()
			while MenuUnicorn.Menu do
                RageUI.IsVisible(RMenu:Get('unicornmenu', 'main'), true, true, true, function()
                    if onService then
                        ...
                        RageUI.List("Annonce", ActionsAnnonce, ActionsAnnonceIndex, nil, {}, true, function(Hovered, Active, Selected, Index) 
                            if (Selected) then 
                                if Index == 1 then 
                                    ...
                                    AtlantisRPEvent('unicorn:announce', announce)
                                elseif Index == 2 then
