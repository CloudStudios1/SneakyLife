            ...
            if ESX.PlayerData.job.name == ConfigBurger.JobName then
                ...
                if not tomate then
                    if #(myCoords-ConfigBurger.Pos.Tomates) < 1.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            ESX.TriggerServerCallback('AtlantisRPburgershot:getItemAmount', function(tomatoes)
                                if tomatoes > 0 then
                                    tomate = true
                                    ...
                                    AtlantisRPEvent("Burgershot:Removeitem","tomate",1,"tomate_coupe",1)
                                else
                                    tomate = false
                                    salade = false
                                    steak = false
                ...
                if not salade then
                    if #(myCoords-ConfigBurger.Pos.Salade) < 1.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            ESX.TriggerServerCallback('AtlantisRPburgershot:getItemAmount', function(salade)
                                if salade > 0 then
                                    salade = true
                                    FreezeEntityPosition(PlayerPedId(), true)
                                    PlayAnim("anim@amb@business@cfm@cfm_cut_sheets@", "cut_guilotine_v1_billcutter", 10000)
                                    ...
                                    FreezeEntityPosition(PlayerPedId(), false)
                                    ClearPedTasks(PlayerPedId())
                                    AtlantisRPEvent("Burgershot:Removeitem","salade",1,"salade_fraiche",1)
                                else
                                    tomate = false
                                    salade = false
                                    steak = false
                ...
                if not frite then
                    if #(myCoords-ConfigBurger.Pos.Frites) < 1.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            ESX.TriggerServerCallback('AtlantisRPburgershot:getItemAmount', function(frites)
                                if frites > 0 then
                                    frite = true
                                    FreezeEntityPosition(PlayerPedId(), true)
                                    ...
                                    FreezeEntityPosition(PlayerPedId(), false)
                                    ClearPedTasks(PlayerPedId())
                                    AtlantisRPEvent("Burgershot:Removeitem","frites",1,"frites_chauffe",1)
                                else
                                    tomate = false
                                    salade = false
                                    steak = false
                ...
                if #(myCoords-ConfigBurger.Pos.Steak) < 1.0 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ESX.TriggerServerCallback('AtlantisRPburgershot:getItemAmount', function(steaks)
                            if steaks > 0 then
                                steak = true
                                FreezeEntityPosition(PlayerPedId(), true)
                                ...
                                FreezeEntityPosition(PlayerPedId(), false)
                                ClearPedTasks(PlayerPedId())
                                AtlantisRPEvent("Burgershot:Removeitem","morviande_2",1,"steaks_cuit",1)
                            else
                                tomate = false
                                salade = false
                                steak = false
                ...
                if #(myCoords-ConfigBurger.Pos.Assemblement) < 1.0 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ESX.TriggerServerCallback('AtlantisRPburgershot:getItemAmount', function(bread)
                            if bread > 0 then
                                burger = true
                                ESX.TriggerServerCallback('AtlantisRPburgershot:getItemAmount', function(salade)
                                    if salade > 0 then
                                        ...
                                        ESX.TriggerServerCallback('AtlantisRPburgershot:getItemAmount', function(steaks)
                                            if steaks > 0 then
                                                steaks = true
                                                ESX.TriggerServerCallback('AtlantisRPburgershot:getItemAmount', function(tomate)
                                                    if tomate > 0 then
                                                        FreezeEntityPosition(PlayerPedId(), true)
                                                        PlayAnim("anim@amb@business@coc@coc_unpack_cut@", "fullcut_cycle_cokecutter", 10000)
                                                        Wait(10000)
                                                        FreezeEntityPosition(PlayerPedId(), false)
                                                        ClearPedTasks(PlayerPedId())
                                                        AtlantisRPEvent("Burgershot:Removeitem","salade_fraiche",1,"tomate_coupe",0)
                                                        AtlantisRPEvent("Burgershot:Removeitem","tomate_coupe",1,"tomate_coupe",0)
                                                        AtlantisRPEvent("Burgershot:Removeitem","steaks_cuit",1,"tomate_coupe",0)
                                                        AtlantisRPEvent("Burgershot:Removeitem","bread",1,"burger",1)
                                                    else
                                                        tomate = false
                                                        salade = false
                                                        steak = false
    ...
    if GarageBurger.Menu then
        ...
        Citizen.CreateThread(function()
            while GarageBurger.Menu do
                RageUI.IsVisible(RMenu:Get('garageburgershot', 'main'), true, true, true, function()
