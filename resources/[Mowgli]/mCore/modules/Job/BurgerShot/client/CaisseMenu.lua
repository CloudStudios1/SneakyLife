...
AddEventHandler('announce:burgershot', function(announce)
    if announce == 'ouvert' then
        ESX.ShowAdvancedNotification("Burger Shot", '~y~Informations', "- Burger Shot ~g~ouvert~s~- Horaire : ~g~Maintenant", "CHAR_BURGERSHOT", 1)
    elseif announce == 'fermeture' then
        ESX.ShowAdvancedNotification("Burger Shot", '~y~Informations', "- Burger Shot ~r~fermé~s~- Horaire : ~g~Maintenant", "CHAR_BURGERSHOT", 1)
    end
end)
                ...
                RageUI.List("Annonce", ActionsAnnonce, ActionsAnnonceIndex, nil, {}, true, function(Hovered, Active, Selected, Index) 
                    if (Selected) then 
                        if Index == 1 then 
                            ...
                            AtlantisRPEvent('burgershot:announce', announce)
                        elseif Index == 2 then
