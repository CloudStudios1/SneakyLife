...
AddEventHandler('announce:yellowjack', function(announce)
    if announce == 'ouvert' then
        ESX.ShowAdvancedNotification('YellowJack', '~y~Informations', "- YellowJack ~g~ouvert~s~- Horaire : ~g~Maintenant", "CHAR_YELLOWJACK", 1)
    elseif announce == 'fermeture' then
        ESX.ShowAdvancedNotification('YellowJack', '~y~Informations', "- YellowJack ~r~fermé~s~- Horaire : ~g~Maintenant", "CHAR_YELLOWJACK", 1)
    end
end)
    ...
    if MenuYellowjack.Menu then
        ...
        Citizen.CreateThread(function()
			while MenuYellowjack.Menu do
                RageUI.IsVisible(RMenu:Get('yellowjackmenu', 'main'), true, true, true, function()
                    if onService then
                        ...
                        RageUI.List("Annonce", ActionsAnnonce, ActionsAnnonceIndex, nil, {}, true, function(Hovered, Active, Selected, Index) 
                            if (Selected) then 
                                if Index == 1 then 
                                    ...
                                    AtlantisRPEvent('yellowjack:announce', announce)
                                elseif Index == 2 then
    ...
    if GarageYellowJack.Menu then
        ...
        Citizen.CreateThread(function()
            while GarageYellowJack.Menu do
                RageUI.IsVisible(RMenu:Get('garageyellowjack', 'main'), true, true, true, function()
