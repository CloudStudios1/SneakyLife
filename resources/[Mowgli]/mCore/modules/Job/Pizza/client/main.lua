            ...
            if ESX.PlayerData.job.name == ConfigPizza.JobName then
                ...
                if not tomate then
                    if #(myCoords-ConfigPizza.Pos.Tomates) < 1.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            ESX.TriggerServerCallback('AtlantisRPpizza:getItemAmount', function(tomatoes)
                                if tomatoes > 0 then
                                    tomate = true
                                    ...
                                    AtlantisRPEvent("Pizza:Removeitem","tomate",1,"tomate_coupe",1)
                                else
                                    tomate = false
                                    steak = false
                                    pate = false
                ...
                if not pate then
                    if #(myCoords-ConfigPizza.Pos.Pates) < 1.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            ESX.TriggerServerCallback('AtlantisRPpizza:getItemAmount', function(pate)
                                if pate > 0 then
                                    pate = true
                                    FreezeEntityPosition(PlayerPedId(), true)
                                    ...
                                    FreezeEntityPosition(PlayerPedId(), false)
                                    ClearPedTasks(PlayerPedId())
                                    AtlantisRPEvent("Pizza:Removeitem","pate",1,"spaghetti_bolognaise",1)
                                else
                                    tomate = false
                                    steak = false
                                    pate = false
                ...
                if #(myCoords-ConfigPizza.Pos.Steak) < 1.0 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ESX.TriggerServerCallback('AtlantisRPpizza:getItemAmount', function(steaks)
                            if steaks > 0 then
                                steak = true
                                FreezeEntityPosition(PlayerPedId(), true)
                                ...
                                FreezeEntityPosition(PlayerPedId(), false)
                                ClearPedTasks(PlayerPedId())
                                AtlantisRPEvent("Pizza:Removeitem","morviande_2",1,"steaks_cuit",1)
                            else
                                tomate = false
                                steak = false
                                pate = false
                ...
                if #(myCoords-ConfigPizza.Pos.Assemblement) < 1.0 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ESX.TriggerServerCallback('AtlantisRPpizza:getItemAmount', function(bread)
                            if bread > 0 then
                                pizza = true
                                ESX.TriggerServerCallback('AtlantisRPpizza:getItemAmount', function(steaks)
                                    if steaks > 0 then
                                        steaks = true
                                        ESX.TriggerServerCallback('AtlantisRPpizza:getItemAmount', function(tomate)
                                            if tomate > 0 then
                                                FreezeEntityPosition(PlayerPedId(), true)
                                                PlayAnim("anim@amb@business@coc@coc_unpack_cut@", "fullcut_cycle_cokecutter", 10000)
                                                Wait(10000)
                                                FreezeEntityPosition(PlayerPedId(), false)
                                                ClearPedTasks(PlayerPedId())
                                                AtlantisRPEvent("Pizza:Removeitem","tomate_coupe",1,"tomate_coupe",0)
                                                AtlantisRPEvent("Pizza:Removeitem","steaks_cuit",1,"tomate_coupe",0)
                                                AtlantisRPEvent("Pizza:Removeitem","bread",1,"pizza",1)
                                            else
                                                tomate = false
                                                steak = false
                                                pate = false
    ...
    if GaragePizza.Menu then
        ...
        Citizen.CreateThread(function()
            while GaragePizza.Menu do
                RageUI.IsVisible(RMenu:Get('garagepizza', 'main'), true, true, true, function()
