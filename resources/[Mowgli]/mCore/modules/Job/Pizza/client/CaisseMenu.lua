...
AddEventHandler('announce:pizza', function(announce)
    if announce == 'ouvert' then
        ESX.ShowAdvancedNotification("~g~Drusilla's~s~ Pizza", '~y~Informations', "- Drusilla's Pizza ~g~ouvert~s~- Horaire : ~g~Maintenant", "CHAR_PIZZA", 1)
    elseif announce == 'fermeture' then
        ESX.ShowAdvancedNotification("~g~Drusilla's~s~ Pizza", '~y~Informations', "- Drusilla's Pizza ~r~fermé~s~- Horaire : ~g~Maintenant", "CHAR_PIZZA", 1)
    end
end)
            ...
            RageUI.List("Annonce", ActionsAnnonce, ActionsAnnonceIndex, nil, {}, true, function(Hovered, Active, Selected, Index) 
                if (Selected) then 
                    if Index == 1 then 
                        ...
                        AtlantisRPEvent('pizza:announce', announce)
                    elseif Index == 2 then
