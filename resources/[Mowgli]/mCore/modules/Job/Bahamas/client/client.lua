...
AddEventHandler('announce:bahamas', function(announce)
    if announce == 'ouvert' then
        ESX.ShowAdvancedNotification('Bahamas Mamas', '~b~Informations', "- Bahamas Mamas ~g~ouvert~s~- Horaire : ~g~Maintenant", "CHAR_BAHAMAS", 1)
    elseif announce == 'fermeture' then
        ESX.ShowAdvancedNotification('Bahamas Mamas', '~b~Informations', "- Bahamas Mamas ~r~fermé~s~- Horaire : ~g~Maintenant", "CHAR_BAHAMAS", 1)
    end
end)
...
local MenuBahamas = {}
RMenu.Add('bahamasmenu', 'main', RageUI.CreateMenu("", "Bahamas ~b~Mamas", nil, nil,"root_cause","bahamas"))
    ...
    if MenuBahamas.Menu then
        ...
        Citizen.CreateThread(function()
			while MenuBahamas.Menu do
                RageUI.IsVisible(RMenu:Get('bahamasmenu', 'main'), true, true, true, function()
                    if onService then
                        ...
                        RageUI.List("Annonce", ActionsAnnonce, ActionsAnnonceIndex, nil, {}, true, function(Hovered, Active, Selected, Index) 
                            if (Selected) then 
                                if Index == 1 then 
                                    ...
                                    AtlantisRPEvent('bahamas:announce', announce)
                                elseif Index == 2 then
...
function OpenBahamasActionMenuRageUIMenu()
    local mainMenu = RageUI.CreateMenu("", "Bahamas ~b~Mamas", 0, 0,"root_cause","bahamas")
...
RMenu:Get('garagebahamas', 'main'):SetSubtitle("Bahamas ~b~Mamas")
    ...
    if GarageBahamas.Menu then
        ...
        Citizen.CreateThread(function()
            while GarageBahamas.Menu do
                RageUI.IsVisible(RMenu:Get('garagebahamas', 'main'), true, true, true, function()
    ...
    while true do
        ...
        if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'bahamas' then
            if #(pCoords-vector3(-1389.2504882812,-592.14727783203,30.319570541382)) < 2.0 then
                att = 1
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~b~aller~s~ au bar.")
                if IsControlJustPressed(0, 51) then
            ...
            if #(pCoords-vector3(-1385.1645507812,-606.48913574219,30.31954574585)) < 2.0 then
                att = 1
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~b~sortir~s~ du bar.")
                if IsControlJustPressed(0, 51) then
