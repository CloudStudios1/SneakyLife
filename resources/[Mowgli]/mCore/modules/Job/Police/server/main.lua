...
RegisterServerEvent("SneakyLife:Message")
AddEventHandler("SneakyLife:Message", function(job,msg)
	local _src = source
	TriggerEvent("ratelimit", _src, "SneakyLife:Message")
	if source then
		local xPlayer, tPlayer = ESX.GetPlayerFromId(source), ESX.GetPlayerFromId(target)
		if authorizeJob[xPlayer.job.name] == nil then
        --[[banPlayerAC(xPlayer.source, {
		...
		TriggerClientEvent("SneakyLife:ClientMessage", -1, job, msg)
	end
end)
