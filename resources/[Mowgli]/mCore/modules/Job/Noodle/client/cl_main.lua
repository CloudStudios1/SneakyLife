...
AddEventHandler('announce:noodlee', function(announce)
    if announce == 'ouvert' then
        ESX.ShowAdvancedNotification('Noodle Exchange', '~y~Informations', "- Noodle Exchange ~g~ouvert~s~- Horaire : ~g~Maintenant", "CHAR_NOODLE", 1)
    elseif announce == 'fermeture' then
        ESX.ShowAdvancedNotification('Noodle Exchange', '~y~Informations', "- Noodle Exchange ~r~fermé~s~- Horaire : ~g~Maintenant", "CHAR_NOODLE", 1)
    end
end)
    ...
    if MenuNoodle.Menu then
        ...
        Citizen.CreateThread(function()
			while MenuNoodle.Menu do
                RageUI.IsVisible(RMenu:Get('noodlemenu', 'main'), true, true, true, function()
                    if onService then
                        ...
                        RageUI.List("Annonce", ActionsAnnonce, ActionsAnnonceIndex, nil, {}, onService, function(Hovered, Active, Selected, Index) 
                            if (Selected) then 
                                if Index == 1 then 
                                    ...
                                    AtlantisRPEvent('noodle:announce', announce)
                                elseif Index == 2 then
    ...
    if GarageNoodle.Menu then
        ...
        Citizen.CreateThread(function()
            while GarageNoodle.Menu do
                RageUI.IsVisible(RMenu:Get('garagenoodle', 'main'), true, true, true, function()
