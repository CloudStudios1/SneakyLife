    ...
    if AutoEcole.Menu then
        ...
        Citizen.CreateThread(function()
			while AutoEcole.Menu do
                ...
                RageUI.IsVisible(RMenu:Get('autoecole', 'question1'), true, true, true, function()
                    if question == 1 then
                    ...
                    elseif question == 6 then
                        if error > 2 then
                            ...
                            RageUI.Button("Résultat de votre examen : "..resultatexamen, nil, { RightLabel = "Récupérer" }, true, function(Hovered, Active, Selected)
                                if Selected then
                                    AtlantisRPEvent("Sneakyesx_dmvschool:addLicense", GetEntityCoords(PlayerPedId()), 'dmv')
...
function StartDriveTest(type)
	ESX.Game.SpawnVehicle(VehicleModels[type], Zones.VehicleSpawnPoint.Pos, 317.0, function(vehicle)
		...
		AtlantisRPEvent('Sneakyesx_vehiclelock:givekey', 'no', newPlate)
...
function StopDriveTest(success)
	if success then
        AtlantisRPEvent("Sneakyesx_dmvschool:addLicense", GetEntityCoords(PlayerPedId()), typekodalegrosfdp)
